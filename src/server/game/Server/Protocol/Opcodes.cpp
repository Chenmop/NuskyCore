...
void OpcodeTable::ValidateAndSetOpcode<true, true>(uint16 opcode, uint16 opcodeNumber, char const* name, SessionStatus status, PacketProcessing processing, pOpcodeHandler handler)
{
    ...
    {
        NC_LOG_ERROR("network", "Tried to override handler of %s with %s (opcode %u)", _internalTable[opcode]->Name, name, opcodeNumber);
        return;
    }
...
{
    NC_LOG_ERROR("network", "Tried to set handler for an invalid opcode %d", opcodeNumber);
}
...
{
    NC_LOG_ERROR("network", "Opcode %s got value 0", name);
}
...
#define DEFINE_OPCODE_HANDLER(opcode, opcodeNumber, status, processing, handler)                                      \
	...
	DEFINE_OPCODE_HANDLER(CMSG_SAVE_CUF_PROFILES,                  0x0C88, STATUS_LOGGEDIN,  PROCESS_THREADUNSAFE, &WorldSession::HandleSaveCUFProfiles              ); // 5.4.8 18414
    ...
    DEFINE_OPCODE_HANDLER(CMSG_BATTLE_PAY_GET_PRODUCT_LIST, 0x0DE0, STATUS_UNHANDLED, PROCESS_THREADUNSAFE, &WorldSession::Handle_NULL);
    ...
    DEFINE_OPCODE_HANDLER(CMSG_LFG_JOIN, 0x046B, STATUS_UNHANDLED,  PROCESS_THREADUNSAFE, &WorldSession::HandleLfgJoinOpcode                ); 
...
#define DEFINE_OPCODE_HANDLER(opcode, opcodeNumber, status) ValidateAndSetOpcode<(opcodeNumber < NUM_OPCODE_HANDLERS), (opcode != 0)>(opcode, opcodeNumber, #opcode, status, PROCESS_INPLACE, 0);
    ...
    //DEFINE_OPCODE_HANDLER(SMSG_UPDATE_ACCOUNT_DATA_COMPLETE,       0x0000, STATUS_NEVER    ); // 5.4.8 18414
