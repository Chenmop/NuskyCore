    ...
    _RBACData(NULL)
{
    ...
    if (z_res != Z_OK)
        NC_LOG_ERROR("network", "Can't initialize packet compression (zlib: deflateInit) Error code: %i (%s)", z_res, zError(z_res));
}
...
WorldSession::~WorldSession()
{
    ...
    if (z_res != Z_OK && z_res != Z_DATA_ERROR) // Z_DATA_ERROR signals that internal state was BUSY
        NC_LOG_ERROR("network", "Can't close packet compression stream (zlib: deflateEnd) Error code: %i (%s)", z_res, zError(z_res));
...
void WorldSession::SendPacket(WorldPacket const* packet, bool forced /*= false*/)
{
    ...
    {
        NC_LOG_ERROR("network.opcode", "Prevented sending of NULL_OPCODE to %s", GetPlayerInfo().c_str());
        return;
    }
    else if (packet->GetOpcode() == UNKNOWN_OPCODE)
    {
        NC_LOG_ERROR("network.opcode", "Prevented sending of UNKNOWN_OPCODE to %s", GetPlayerInfo().c_str());
        return;
    }
    ...
    if (!forced)
    {
        ...
        if (!handler || handler->Status == STATUS_UNHANDLED)
        {
            if (packet->GetOpcode() == NULL_OPCODE)
            {
                NC_LOG_ERROR("network.opcode", "Prevented sending disabled opcode %s to %s", GetOpcodeNameForLogging(packet->GetOpcode(), true).c_str(), GetPlayerInfo().c_str());

            }
            else
            {
                NC_LOG_ERROR("network.opcode", "Disabled opcode %s have opcode value, but is disabled missing structure update?", GetOpcodeNameForLogging(packet->GetOpcode(), true).c_str());
            }
            return;
        }
    }
    ...
    else
    {
        ...
        NC_LOG_INFO("misc", "Send all time packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f time: %u", sendPacketCount, sendPacketBytes, float(sendPacketCount)/fullTime, float(sendPacketBytes)/fullTime, uint32(fullTime));
        NC_LOG_INFO("misc", "Send last min packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f", sendLastPacketCount, sendLastPacketBytes, float(sendLastPacketCount)/minTime, float(sendLastPacketBytes)/minTime);
...
{
    NC_LOG_ERROR("network.opcode", "Received unexpected opcode %s Status: %s Reason: %s from %s",
...
void WorldSession::LogUnprocessedTail(WorldPacket* packet)
{
    ...
    NC_LOG_TRACE("network.opcode", "Unprocessed tail data (read stop at %u from %u) Opcode %s from %s",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
            ...
            _recvQueue.next(packet, updater))
    {
        ...
        try
        {
            switch (opHandle->Status)
            {
                case STATUS_LOGGEDIN:
                    if (!_player)
                    {
                        ...
                        if (!m_playerRecentlyLogout)
                        {
                            ...
                            QueuePacket(packet);
                            //! Log
                                NC_LOG_DEBUG("network", "Re-enqueueing packet with opcode %s with with status STATUS_LOGGEDIN. "
                ...
                case STATUS_AUTHED:
                    ...
                    break;
                case STATUS_NEVER:
                        NC_LOG_ERROR("network.opcode", "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode(), false).c_str(),
                            GetPlayerInfo().c_str());
                    break;
                case STATUS_UNHANDLED:
                        NC_LOG_ERROR("network.opcode", "Received not handled opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode(), false).c_str(),
                            GetPlayerInfo().c_str());
                    break;
            }
        }
        ...
        {
            NC_LOG_ERROR("network", "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %s) from client %s, accountid=%i. Skipped packet.",
    ...
    if (_player)
    {
        ...
        for (int i=0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
            if (BattlegroundQueueTypeId bgQueueTypeId = _player->GetBattlegroundQueueTypeId(i))
            {
                ...
                if(_player->IsInvitedForBattlegroundInstance(_player->GetBattlegroundId()) && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_TRACK_DESERTERS))
                {
        ...
        NC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Logout Character:[%s] (GUID: %u) Level: %d",
        ...
        NC_LOG_DEBUG("network", "SESSION: Sent SMSG_LOGOUT_COMPLETE Message");
...
{
    char const* format = GetNuskyString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetNuskyString(int32 entry) const
{
    return sObjectMgr->GetNuskyString(entry, GetSessionDbLocaleIndex());
}

void WorldSession::Handle_NULL(WorldPacket& recvPacket)
{
    NC_LOG_ERROR("network.opcode", "Received unhandled opcode %s from %s",
        GetOpcodeNameForLogging(recvPacket.GetOpcode(), false).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    NC_LOG_ERROR("network.opcode", "Received opcode %s that must be processed in WorldSocket::OnRead from %s",
        GetOpcodeNameForLogging(recvPacket.GetOpcode(), false).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_EarlyProccessContinued(WorldPacket& recvPacket) //CMSG_AUTH_CONTINUED_SESSION(void *this, int a2)
{
	NC_LOG_ERROR("network.opcode", "Recived opcode %s that must be processed in WorldSocket::Unknown from %s",
	    GetOpcodeNameForLogging(recvPacket.GetOpcode(), false).c_str(), GetPlayerInfo().c_str());
...
{
    NC_LOG_ERROR("network.opcode", "Received deprecated opcode %s from %s",
        GetOpcodeNameForLogging(recvPacket.GetOpcode(), false).c_str(), GetPlayerInfo().c_str());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            NC_LOG_ERROR("misc", "Table `%s` have invalid account data type (%u), ignore.",
        ...
        if ((mask & (1 << type)) == 0)
        {
            NC_LOG_ERROR("misc", "Table `%s` have non appropriate for table  account data type (%u), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadAddonsInfo(WorldPacket &data)
{
    ...
    if (size > 0xFFFFF)
    {
        NC_LOG_ERROR("misc", "WorldSession::ReadAddonsInfo addon info too big, size %u", size);
        return;
    }
    ...
    if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos, data.size() - pos) == Z_OK)
    {
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            NC_LOG_INFO("misc", "ADDON: Name: %s, UsePubKey: 0x%x, CRC: 0x%x, UrlFile: %i", addonName.c_str(), usingPubKey, crc, urlFile);
            ...
            if (savedAddon)
            {
                if (addon.CRC != savedAddon->CRC)
                    NC_LOG_INFO("misc", "ADDON: %s was known, but didn't match known CRC (0x%x)!", addon.Name.c_str(), savedAddon->CRC);
                else
                    NC_LOG_INFO("misc", "ADDON: %s was known, CRC is correct (0x%x)", addon.Name.c_str(), savedAddon->CRC);
            }
            else
            {
                AddonMgr::SaveAddon(addon);

                NC_LOG_INFO("misc", "ADDON: %s (0x%x) was not known, saving...", addon.Name.c_str(), addon.CRC);
            }
        ...
        NC_LOG_DEBUG("network", "ADDON: CurrentTime: %u", currentTime);
    }
    else
        NC_LOG_ERROR("misc", "Addon packet uncompress error!");
}
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_UNREGISTER_ALL_ADDON_PREFIXES");
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ADDON_REGISTERED_PREFIXES");
...
void WorldSession::LoadPermissions()
{
    ...
    NC_LOG_DEBUG("rbac", "WorldSession::LoadPermissions [AccountId: %u, Name: %s, realmId: %d, secLevel: %u]",
...
bool WorldSession::HasPermission(uint32 permission)
{
    ...
    NC_LOG_DEBUG("rbac", "WorldSession::HasPermission [AccountId: %u, Name: %s, realmId: %d]",
...
{
    NC_LOG_DEBUG("rbac", "WorldSession::Invalidaterbac::RBACData [AccountId: %u, Name: %s, realmId: %d]",
                   _RBACData->GetId(), _RBACData->GetName().c_str(), realmID);
    delete _RBACData;
    _RBACData = NULL;
}
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket& p) const
{
    ...
    NC_LOG_INFO("network", "AntiDOS: Account %u, IP: %s, sent unacceptable packet (opc: %u, size: %u)",
    ...
    switch (_policy)
    {
        case POLICY_LOG:
            return true;
        case POLICY_KICK:
            NC_LOG_INFO("network", "AntiDOS: Player kicked!");
            return false;
        case POLICY_BAN:
        {
            ...
            NC_LOG_INFO("network", "AntiDOS: Player automatically banned for %u seconds.", duration);
