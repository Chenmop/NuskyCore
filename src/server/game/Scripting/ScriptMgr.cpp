    ...
    public:
        ...
        static void AddScript(TScript* const script)
        {
            ...
            for (ScriptMapIterator it = ScriptPointerList.begin(); it != ScriptPointerList.end(); ++it)
            {
                if (it->second == script)
                {
                    NC_LOG_ERROR("scripts", "Script '%s' has same memory pointer as '%s'.",
            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    else
                    {
                        ...
                        NC_LOG_ERROR("scripts", "Script '%s' already assigned with the same script name, so the script can't work.",
                ...
                else
                {
                    ...
                    if (script->GetName().find("example") == std::string::npos && script->GetName().find("Smart") == std::string::npos)
                        NC_LOG_ERROR("sql.sql", "Script named '%s' does not have a script name assigned in database.",
                            script->GetName().c_str());
...
void ScriptMgr::Initialize()
{
    ...
    NC_LOG_INFO("server.loading", "Loading C++ scripts");

    FillSpellSummary();
    AddScripts();

    NC_LOG_INFO("server.loading", ">> Loaded %u C++ scripts in %u ms", GetScriptCount(), GetMSTimeDiffToNow(oldMSTime));
}
    ...
    : ScriptObject(name), MapScript<Map>(mapId)
{
    if (GetEntry() && !GetEntry()->IsWorldMap())
        NC_LOG_ERROR("scripts", "WorldMapScript for map %u is invalid.", mapId);
    ...
    : ScriptObject(name), MapScript<InstanceMap>(mapId)
{
    if (GetEntry() && !GetEntry()->IsInstance())
        NC_LOG_ERROR("scripts", "InstanceMapScript for map %u is invalid.", mapId);
    ...
    : ScriptObject(name), MapScript<BattlegroundMap>(mapId)
{
    if (GetEntry() && !GetEntry()->IsBattleground())
        NC_LOG_ERROR("scripts", "BattlegroundMapScript for map %u is invalid.", mapId);
