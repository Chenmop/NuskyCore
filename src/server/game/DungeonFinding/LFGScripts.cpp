...
void LFGPlayerScript::OnLogin(Player* player)
{
    ...
    if (Group const* group = player->GetGroup())
    {
        ...
        if (gguid != gguid2)
        {
            NC_LOG_ERROR("lfg", "%s on group %u but LFG has group %u saved... Fixing.",
...
void LFGPlayerScript::OnMapChanged(Player* player)
{
    ...
    if (sLFGMgr->inLfgDungeonMap(player->GetGUID(), map->GetId(), map->GetDifficulty()))
    {
        ...
        if (!group)
        {
            ...
            NC_LOG_ERROR("lfg", "LFGPlayerScript::OnMapChanged, Player %s (%u) is in LFG dungeon map but does not have a valid group! "
...
void LFGGroupScript::OnAddMember(Group* group, uint64 guid)
{
    ...
    if (leader == guid)
    {
        NC_LOG_DEBUG("lfg", "LFGScripts::OnAddMember [" UI64FMTD "]: added [" UI64FMTD "] leader " UI64FMTD "]", gguid, guid, leader);
    ...
    else
    {
        ...
        NC_LOG_DEBUG("lfg", "LFGScripts::OnAddMember [" UI64FMTD "]: added [" UI64FMTD "] leader " UI64FMTD "] gstate: %u, state: %u", gguid, guid, leader, gstate, state);
...
void LFGGroupScript::OnRemoveMember(Group* group, uint64 guid, RemoveMethod method, uint64 kicker, char const* reason)
{
    ...
    uint64 gguid = group->GetGUID();
    NC_LOG_DEBUG("lfg", "LFGScripts::OnRemoveMember [" UI64FMTD "]: remove [" UI64FMTD "] Method: %d Kicker: [" UI64FMTD "] Reason: %s", gguid, guid, method, kicker, (reason ? reason : ""));
...
void LFGGroupScript::OnDisband(Group* group)
{
    ...
    uint64 gguid = group->GetGUID();
    NC_LOG_DEBUG("lfg", "LFGScripts::OnDisband [" UI64FMTD "]", gguid);
...
void LFGGroupScript::OnChangeLeader(Group* group, uint64 newLeaderGuid, uint64 oldLeaderGuid)
{
    ...
    NC_LOG_DEBUG("lfg", "LFGScripts::OnChangeLeader [" UI64FMTD "]: old [" UI64FMTD "] new [" UI64FMTD "]", gguid, newLeaderGuid, oldLeaderGuid);
...
void LFGGroupScript::OnInviteMember(Group* group, uint64 guid)
{
    ...
    uint64 gguid = group->GetGUID();
    uint64 leader = group->GetLeaderGUID();
    NC_LOG_DEBUG("lfg", "LFGScripts::OnInviteMember [" UI64FMTD "]: invite [" UI64FMTD "] leader [" UI64FMTD "]", gguid, guid, leader);
