...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS]=
{
    ...
    &Spell::EffectTriggerSpell,                             //160 SPELL_EFFECT_TRIGGER_SPELL_3
    ...
    &Spell::EffectSummonObject,                             //171 SPELL_EFFECT_SUMMON_OBJECT_SLOT_2
...
{
    NC_LOG_DEBUG("spells", "WORLD: Spell Effect DUMMY");
}

void Spell::EffectUnused(SpellEffIndex /*effIndex*/)
{
    // NOT USED BY ANY SPELL OR USELESS OR IMPLEMENTED IN DIFFERENT WAY IN Nusky
}
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
        ...
        case SPELLFAMILY_PALADIN:
            switch (m_spellInfo->Id)
            {
                case 31789:                                 // Righteous Defense (step 1)
                {
                    ...
                    {
                        Unit* attacker = Nusky::Containers::SelectRandomContainerElement(attackers);
    ...
    if (spell_id)
    {
        ...
        if (!spellInfo)
        {
            NC_LOG_ERROR("spells", "EffectDummy of spell %u: triggering unknown spell id %i\n", m_spellInfo->Id, spell_id);
            return;
        }
    ...
    NC_LOG_DEBUG("spells", "Spell ScriptStart spellid %u in EffectDummy(%u)", m_spellInfo->Id, effIndex);
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        NC_LOG_DEBUG("spells", "Spell::EffectTriggerSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        NC_LOG_DEBUG("spells", "Spell::EffectTriggerMissileSpell spell %u tried to trigger unknown spell %u", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        NC_LOG_ERROR("spells", "Spell::EffectForceCast of spell %u: triggering unknown spell id %i", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        NC_LOG_ERROR("spells", "EffectTriggerRitualOfSummoning of spell %u: triggering unknown spell id %i", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_targets.HasDst())
    {
        NC_LOG_ERROR("spells", "Spell::EffectTeleportUnits - does not have destination for spellId %u.", m_spellInfo->Id);
        return;
    }
    ...
    NC_LOG_DEBUG("spells", "Spell::EffectTeleportUnits - teleport unit to %u %f %f %f %f\n", mapid, x, y, z, orientation);
    ...
    {
        NC_LOG_ERROR("spells", "Spell::EffectTeleportUnits - spellId %u attempted to teleport creature to a different map.", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
    ...
    NC_LOG_DEBUG("spells", "Spell: Player %u has unlearned spell %u from NpcGUID: %u", player->GetGUIDLow(), spellToUnlearn, m_caster->GetGUIDLow());
}
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
    ...
    NC_LOG_DEBUG("spells", "Spell ScriptStart %u for spellid %u in EffectSendEvent ", m_spellInfo->Effects[effIndex].MiscValue, m_spellInfo->Id);
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            if (!targetAura)
            {
                NC_LOG_ERROR("spells", "Target(GUID:" UI64FMTD ") has aurastate AURA_STATE_SWIFTMEND but no matching aura.", unitTarget->GetGUID());
                return;
            }
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    NC_LOG_DEBUG("spells", "HealthLeech :%i", damage);
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        if (!avalibleElixirs.empty())
        {
            ...
            m_caster->CastSpell(unitTarget, Nusky::Containers::SelectRandomContainerElement(avalibleElixirs), true, m_CastItem);
        }
    }
}
...
void Spell::SendLoot(uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
            NC_LOG_ERROR("spells", "Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!",
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_CHEST:
                ...
                {
                    NC_LOG_DEBUG("spells", "Chest ScriptStart id %u for GO %u", gameObjTarget->GetGOInfo()->chest.eventId, gameObjTarget->GetDBTableGUIDLow());
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() != TYPEID_PLAYER)
    {
        NC_LOG_DEBUG("spells", "WORLD: Open Lock - No Player Caster!");
        return;
    }
    ...
    {
        NC_LOG_DEBUG("spells", "WORLD: Open Lock - No GameObject/Item Target!");
        return;
    }
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    if (!properties)
    {
        NC_LOG_ERROR("spells", "EffectSummonType: Unhandled summon type %u", m_spellInfo->Effects[effIndex].MiscValueB);
        return;
    }
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
    ...
    NC_LOG_DEBUG("spells", "Spell: Player %u has learned spell %u from NpcGUID=%u", player->GetGUIDLow(), spellToLearn, m_caster->GetGUIDLow());
}
...
void Spell::EffectEnchantItemPerm(SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        if (!enchant_id)
        {
            NC_LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_PERMANENT) have 0 as enchanting id", m_spellInfo->Id, effIndex);
            return;
        }
        ...
        if (!pEnchant)
        {
            NC_LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_PERMANENT) have non-existing enchant_id %u ", m_spellInfo->Id, effIndex, enchant_id);
            return;
        }
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            NC_LOG_ERROR("spells", "Spell::EffectEnchantItemPrismatic: attempt apply enchant spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u) but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not suppoted yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (!enchant_id)
    {
        NC_LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have 0 as enchanting id", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    if (!pEnchant)
    {
        NC_LOG_ERROR("spells", "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have not existed enchanting id %u ", m_spellInfo->Id, effIndex, enchant_id);
        return;
    }
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex)
{
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 60123: // Lightwell
                {
                    ...
                    switch (m_caster->GetEntry())
                    {
                        ...
                        default:
                            NC_LOG_ERROR("spells", "Unknown Lightwell spell caster %u", m_caster->GetEntry());
                            return;
                    }
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (m_spellInfo->GetCategory() == SPELLCATEGORY_JUDGEMENT)
            {
                ...
                switch (m_spellInfo->Id)
                {
                    ...
                    default:
                        NC_LOG_ERROR("spells", "Unsupported Judgement (seal trigger) spell (Id: %u) in Spell::EffectScriptEffect", m_spellInfo->Id);
                        return;
                }
    ...
    // normal DB scripted effect
    NC_LOG_DEBUG("spells", "Spell ScriptStart spellid %u in EffectScriptEffect(%u)", m_spellInfo->Id, effIndex);
    m_caster->GetMap()->ScriptsStart(sSpellScripts, uint32(m_spellInfo->Id | (effIndex << 24)), m_caster, unitTarget);
}
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    NC_LOG_DEBUG("spells", "Spell Effect: Stuck");
    NC_LOG_INFO("spells", "Player %s (guid %u) used auto-unstuck future at map %u (%f, %f, %f)", player->GetName().c_str(), player->GetGUIDLow(), player->GetMapId(), player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (!goinfo)
    {
        NC_LOG_ERROR("sql.sql", "Gameobject (Entry: %u) not exist and not created at spell (ID: %u) cast", name_id, m_spellInfo->Id);
        return;
    }
    ...
    NC_LOG_DEBUG("spells", "AddObject at SpellEfects.cpp EffectTransmitted");
    //m_caster->AddGameObject(pGameObj);
...
void Spell::EffectSkill(SpellEffIndex /*effIndex*/)
{
    ...
    NC_LOG_DEBUG("spells", "WORLD: SkillEFFECT");
}
...
void Spell::EffectSkinPlayerCorpse(SpellEffIndex /*effIndex*/)
{
    ...
    NC_LOG_DEBUG("spells", "Effect: SkinPlayerCorpse");
...
void Spell::EffectStealBeneficialBuff(SpellEffIndex effIndex)
{
    ...
    NC_LOG_DEBUG("spells", "Effect: StealBeneficialBuff");
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex)
{
    ...
    {
        NC_LOG_ERROR("spells", "EffectPlayMusic: Sound (Id: %u) not exist in spell %u.", SoundKitID, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPlaySound(SpellEffIndex effIndex)
{
    ...
    {
        NC_LOG_ERROR("spells", "EffectPlaySound: Sound (Id: %u) not exist in spell %u.", soundId, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectBind(SpellEffIndex effIndex)
{
    ...
    NC_LOG_DEBUG("spells", "EffectBind: New homebind X: %f, Y: %f, Z: %f, MapId: %u, AreaId: %u",
