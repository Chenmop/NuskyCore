...
{
    if (!m_targetMask)
        NC_LOG_INFO("spells", "No targets");

    NC_LOG_INFO("spells", "target mask: %u", m_targetMask);
    if (m_targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK | TARGET_FLAG_GAMEOBJECT_MASK))
        NC_LOG_INFO("spells", "Object target: " UI64FMTD, m_objectTargetGUID);
    if (m_targetMask & TARGET_FLAG_ITEM)
        NC_LOG_INFO("spells", "Item target: " UI64FMTD, m_itemTargetGUID);
    if (m_targetMask & TARGET_FLAG_TRADE_ITEM)
        NC_LOG_INFO("spells", "Trade item target: " UI64FMTD, m_itemTargetGUID);
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
        NC_LOG_INFO("spells", "Source location: transport guid:" UI64FMTD " trans offset: %s position: %s", m_src._transportGUID, m_src._transportOffset.ToString().c_str(), m_src._position.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
        NC_LOG_INFO("spells", "Destination location: transport guid:" UI64FMTD " trans offset: %s position: %s", m_dst._transportGUID, m_dst._transportOffset.ToString().c_str(), m_dst._position.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_STRING)
        NC_LOG_INFO("spells", "String: %s", m_strTarget.c_str());
    NC_LOG_INFO("spells", "speed: %f", m_speed);
    NC_LOG_INFO("spells", "elevation: %f", m_elevation);
}
...
Spell::~Spell()
{
    ...
    if (m_referencedFromCurrentSpell && m_selfContainer && *m_selfContainer == this)
    {
        ...
        NC_LOG_ERROR("spells", "SPELL: deleting spell for spell ID %u. However, spell still referenced.", m_spellInfo->Id);
        *m_selfContainer = NULL;
    }
...
void Spell::SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32& processedEffectMask)
{
    ...
    switch (targetType.GetSelectionCategory())
    {
        ...
        case TARGET_SELECT_CATEGORY_NYI:
            NC_LOG_DEBUG("spells", "SPELL: target type %u, found in spellID %u, effect %u is not implemented yet!", m_spellInfo->Id, effIndex, targetType.GetTarget());
            break;
        default:
...
void Spell::SelectImplicitChannelTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    if (!channeledSpell)
    {
        NC_LOG_DEBUG("spells", "Spell::SelectImplicitChannelTargets: cannot find channel spell for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
        return;
    }
    switch (targetType.GetTarget())
    {
        case TARGET_UNIT_CHANNEL_TARGET:
        {
            ...
            if (target && target->ToUnit())
                ...
                NC_LOG_DEBUG("spells", "SPELL: cannot find channel spell target for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
            break;
        }
        case TARGET_DEST_CHANNEL_TARGET:
            ...
            else if (WorldObject* target = ObjectAccessor::GetWorldObject(*m_caster, m_originalCaster->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT)))
            {
                ...
                NC_LOG_DEBUG("spells", "SPELL: cannot find channel spell destination for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
            break;
...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        NC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: no conditions entry for target with TARGET_CHECK_ENTRY of spell ID %u, effect %u - selecting default targets", m_spellInfo->Id, effIndex);
        switch (targetType.GetObjectType())
        {
    ...
    if (!target)
    {
        NC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: cannot find nearby target for spell ID %u, effect %u", m_spellInfo->Id, effIndex);
        return;
    }
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        Nusky::WorldObjectSpellConeTargetCheck check(coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
        Nusky::WorldObjectListSearcher<Nusky::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
        SearchTargets<Nusky::WorldObjectListSearcher<Nusky::WorldObjectSpellConeTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, radius);
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
                Nusky::Containers::RandomResizeList(targets, maxTargets);
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (!unitTargets.empty())
    {
        ...
        if (maxSize && power != -1)
        {
            if (Powers(power) == POWER_HEALTH)
            {
                ...
                {
                    unitTargets.sort(Nusky::HealthPctOrderPred());
            ...
            else
            {
                ...
                if (unitTargets.size() > maxSize)
                {
                    unitTargets.sort(Nusky::PowerPctOrderPred((Powers)power));
                    unitTargets.resize(maxSize);
                }
            }
        }
        ...
        // Other special target selection goes here
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            Nusky::Containers::RandomResizeList(unitTargets, maxTargets);
    ...
    {
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            Nusky::Containers::RandomResizeList(gObjTargets, maxTargets);
...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_DB:
            ...
            {
                NC_LOG_DEBUG("spells", "SPELL: unknown target coordinates for spell ID %u", m_spellInfo->Id);
...
void Spell::SelectImplicitTrajTargets()
{
    ...
    std::list<WorldObject*> targets;
    Nusky::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo);
    Nusky::WorldObjectListSearcher<Nusky::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<Nusky::WorldObjectListSearcher<Nusky::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(), dist2d);
    if (targets.empty())
        return;

    targets.sort(Nusky::ObjectDistanceOrderPred(m_caster));

    float b = tangent(m_targets.GetElevation());
    float a = (srcToDestDelta - dist2d * b) / (dist2d * dist2d);
    if (a > -0.0001f)
        a = 0;
    DEBUG_TRAJ(NC_LOG_ERROR("spells", "Spell::SelectTrajTargets: a %f b %f", a, b);)
    ...
    for (; itr != targets.end(); ++itr)
    {
        ...
        DEBUG_TRAJ(NC_LOG_ERROR("spells", "Spell::SelectTrajTargets: check %u, dist between %f %f, height between %f %f.", (*itr)->GetEntry(), objDist2d - size, objDist2d + size, dz - size, dz + size);)

        float dist = objDist2d - size;
        float height = dist * (a * dist + b);
        DEBUG_TRAJ(NC_LOG_ERROR("spells", "Spell::SelectTrajTargets: dist %f, height %f.", dist, height);)
...
#define CHECK_DIST {\
    ...
    if (m_targets.GetSrcPos()->GetExactDist2d(m_targets.GetDstPos()) > bestDist)
    {
        ...
        if (itr != targets.end())
        {
            ...
            sizeSq *= sizeSq;
            DEBUG_TRAJ(NC_LOG_ERROR("spells", "Initial %f %f %f %f %f", x, y, z, distSq, sizeSq);)
            if (distSq > sizeSq)
            {
...
void Spell::SearchTargets(SEARCHER& searcher, uint32 containerMask, Unit* referer, Position const* pos, float radius)
{
    ...
    if (searchInGrid || searchInWorld)
    {
        ...
        CellCoord p(Nusky::ComputeCellCoord(x, y));
        Cell cell(p);
        cell.SetNoCreate();
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return NULL;
    Nusky::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo, selectionType, condList);
    Nusky::WorldObjectLastSearcher<Nusky::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check, containerTypeMask);
    SearchTargets<Nusky::WorldObjectLastSearcher<Nusky::WorldObjectSpellNearbyTargetCheck> > (searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return;
    Nusky::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList);
    Nusky::WorldObjectListSearcher<Nusky::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<Nusky::WorldObjectListSearcher<Nusky::WorldObjectSpellAreaTargetCheck> > (searcher, containerTypeMask, m_caster, position, range);
}
...
GameObject* Spell::SearchSpellFocus()
{
    ...
    Nusky::GameObjectFocusCheck check(m_caster, m_spellInfo->RequiresSpellFocus);
    Nusky::GameObjectSearcher<Nusky::GameObjectFocusCheck> searcher(m_caster, focus, check);
    SearchTargets<Nusky::GameObjectSearcher<Nusky::GameObjectFocusCheck> > (searcher, GRID_MAP_TYPE_MASK_GAMEOBJECT, m_caster, m_caster, m_caster->GetVisibilityRange());
    return focus;
}
    ...
    if (!m_hitTriggerSpells.empty())
    {
        ...
        for (HitTriggerSpellList::const_iterator i = m_hitTriggerSpells.begin(); i != m_hitTriggerSpells.end(); ++i)
        {
            if (CanExecuteTriggersOnHit(effMask, i->triggeredByAura) && roll_chance_i(i->chance))
            {
                ...
                NC_LOG_DEBUG("spells", "Spell %d triggered spell %d by SPELL_AURA_ADD_TARGET_TRIGGER aura", m_spellInfo->Id, i->triggeredSpell->Id);
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    NC_LOG_DEBUG("spells", "Spell::prepare: spell id %u source %u caster %d customCastFlags %u mask %u", m_spellInfo->Id, m_caster->GetEntry(), m_originalCaster ? m_originalCaster->GetEntry() : -1, _triggeredCastFlags, m_targets.GetTargetMask());
...
void Spell::update(uint32 difftime)
{
    ...
    {
        NC_LOG_DEBUG("spells", "Spell %u is cancelled due to removal of target.", m_spellInfo->Id);
        cancel();
        return;
    }
    ...
    switch (m_spellState)
    {
        ...
        case SPELL_STATE_CASTING:
        {
            if (m_timer)
            {
                ...
                {
                    NC_LOG_DEBUG("spells", "Channeled spell %d is removed due to lack of targets", m_spellInfo->Id);
                    SendChannelUpdate(0);
                    finish();
                }
...
void Spell::finish(bool ok)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT && m_caster->ToCreature()->IsSummon())
    {
        ...
        {
            NC_LOG_DEBUG("spells", "Statue %d is unsummoned in spell %d finish", m_caster->GetGUIDLow(), m_spellInfo->Id);
...
void Spell::SendSpellGo()
{
    ...
    //NC_LOG_DEBUG("spells", "Sending SMSG_SPELL_GO id=%u", m_spellInfo->Id);
...
void Spell::TakeCastItem()
{
    ...
    if (!proto)
    {
        ...
        NC_LOG_ERROR("spells", "Cast item has no item prototype highId=%d, lowId=%d", m_CastItem->GetGUIDHigh(), m_CastItem->GetGUIDLow());
        return;
    }
...
void Spell::TakePower()
{
    ...
    {
        NC_LOG_ERROR("spells", "Spell::TakePower: Unknown power type '%d'", powerType);
        return;
    }
...
void Spell::HandleThreatSpells()
{
    ...
    NC_LOG_DEBUG("spells", "Spell %u, added an additional %f threat for %s %u target(s)", m_spellInfo->Id, threat, m_spellInfo->_IsPositiveSpell() ? "assisting" : "harming", uint32(m_UniqueTargetInfo.size()));
}
...
void Spell::HandleEffects(Unit* pUnitTarget, Item* pItemTarget, GameObject* pGOTarget, uint32 i, SpellEffectHandleMode mode)
{
    ...
    NC_LOG_DEBUG("spells", "Spell: %u Effect : %u", m_spellInfo->Id, eff);
...
SpellCastResult Spell::CheckPower()
{
    ...
    {
        NC_LOG_ERROR("spells", "Spell::CheckPower: Unknown power type '%d'", m_spellInfo->PowerType);
...
void Spell::Delayed() // only called in DealDamage()
{
    ...
    NC_LOG_INFO("spells", "Spell %u partially interrupted for (%d) ms at damage", m_spellInfo->Id, delaytime);
...
void Spell::DelayedChannel()
{
    ...
    NC_LOG_DEBUG("spells", "Spell %u partially interrupted for %i ms, new duration: %u ms", m_spellInfo->Id, delaytime, m_timer);
...
SpellEvent::~SpellEvent()
{
    ...
    {
        NC_LOG_ERROR("spells", "~SpellEvent: %s %u tried to delete non-deletable spell %u. Was not deleted, causes memory leak.",
...
void Spell::LoadScripts()
{
    ...
    for (std::list<SpellScript*>::iterator itr = m_loadedScripts.begin(); itr != m_loadedScripts.end();)
    {
        ...
        NC_LOG_DEBUG("spells", "Spell::LoadScripts: Script `%s` for spell `%u` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
        (*itr)->Register();
        ++itr;
    }
}
