...
Group::~Group()
{
    if (m_bgGroup)
    {
        NC_LOG_DEBUG("bg.battleground", "Group::~Group: battleground group being deleted.");
        ...
        else NC_LOG_ERROR("misc", "Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //NC_LOG_DEBUG("misc", "Group::GroupLoot: missing item prototype for item with id: %d", i->itemid);
            continue;
        }
...
{
    NC_LOG_DEBUG("network", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST)");
...
InstanceGroupBind* Group::BindToInstance(InstanceSave* save, bool permanent, bool load)
{
    ...
    bind.perm = permanent;
    if (!load)
        NC_LOG_DEBUG("maps", "Group::BindToInstance: Group (guid: %u, storage id: %u) is now bound to map %d, instance %d, difficulty %d",
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (pp && pp->IsInWorld())
        {
            ...
            NC_LOG_DEBUG("misc", "-- Forced group value update for '%s'", pp->GetName().c_str());
        }
    }
}
