...
{
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_REPOP_REQUEST Message");
    ...
    {
        NC_LOG_DEBUG("network", "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated",
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_GOSSIP_SELECT_OPTION");
    ...
    if (IS_CRE_OR_VEH_GUID(guid))
    {
        ...
        if (!unit)
        {
            NC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else if (IS_GAMEOBJECT_GUID(guid))
    {
        ...
        if (!go)
        {
            NC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - GameObject (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else
    {
        NC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - unsupported GUID type for highguid %u. lowpart %u.", uint32(GUID_HIPART(guid)), uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    {
        NC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
        if (unit)
...
{
    //CMSG_WHO
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_WHO Message");
    ...
    for (uint32 i = 0; i < wordCount; ++i)
    {
        ...
        NC_LOG_DEBUG("network", "String %u: %s", i, temp.c_str());
    }
    ...
    for (uint32 i = 0; i < zonesCount; ++i)
    {
        ...
        NC_LOG_DEBUG("network", "Zone %u: %u", i, zoneIds[i]);
    }
    ...
    NC_LOG_DEBUG("network", "Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask %u, zones %u, strings %u", levelMin, levelMax, playerName.c_str(), guildName.c_str(), raceMask, classMask, zonesCount, wordCount);
    ...
    // SMSG_WHO
    // client send in case not set max level value 100 but Nusky supports 255 max level,
    ...
    NC_LOG_DEBUG("network", "WORLD: Send SMSG_WHO Message");
}

void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %u", GetSecurity());
...
{
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_PLAYER_LOGOUT Message");
}

void WorldSession::HandleLogoutCancelOpcode(WorldPacket& /*recvData*/)
{
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_LOGOUT_CANCEL Message");
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_LOGOUT_CANCEL_ACK Message");
}
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Recvd ZONE_UPDATE: %u", newZone);
...
void WorldSession::HandleRequestCemeteryList(WorldPacket& /*recvPacket*/)
{
    ...
    {
        NC_LOG_DEBUG("network", "No graveyards found for zone %u for %u (team %u) in CMSG_REQUEST_CEMETERY_LIST", zoneId, m_GUIDLow, team);
        return;
    }
...
{
    // NC_LOG_DEBUG("network", "WORLD: Received CMSG_STAND_STATE_CHANGE"); -- too many spam in log at lags/debug stop
    uint32 animstate;
    recvData >> animstate;
...
void WorldSession::HandleContactListOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_CONTACT_LIST");
    _player->GetSocial()->SendSocialList(_player);
}

void WorldSession::HandleAddFriendOpcode(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ADD_FRIEND");

    std::string friendName = GetNuskyString(LANG_FRIEND_IGNORE_UNKNOWN);
    ...
    NC_LOG_DEBUG("network", "WORLD: %s asked to add friend : '%s'",
...
void WorldSession::HandleAddFriendOpcodeCallBack(PreparedQueryResult result, std::string const& friendNote)
{
            ...
            if (friendGuid)
            {
                ...
                else
                {
                    ...
                    if (!GetPlayer()->GetSocial()->AddToSocialList(GUID_LOPART(friendGuid), false))
                    {
                        ...
                        NC_LOG_DEBUG("network", "WORLD: %s's friend list is full.", GetPlayer()->GetName().c_str());
                    }
                }
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent (SMSG_FRIEND_STATUS)");
}
...
void WorldSession::HandleDelFriendOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_DEL_FRIEND");
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent motd (SMSG_FRIEND_STATUS)");
}

void WorldSession::HandleAddIgnoreOpcode(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ADD_IGNORE");

    std::string ignoreName = GetNuskyString(LANG_FRIEND_IGNORE_UNKNOWN);
    ...
    NC_LOG_DEBUG("network", "WORLD: %s asked to Ignore: '%s'",
...
void WorldSession::HandleDelIgnoreOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_DEL_IGNORE");
...
{
    NC_LOG_DEBUG("network", "CMSG_SET_CONTACT_NOTES");
    uint64 guid;
    std::string note;
    recvData >> guid >> note;
...
void WorldSession::HandleBugOpcode(WorldPacket& recvData)
{
    ...
    if (suggestion == 0)
        NC_LOG_DEBUG("network", "WORLD: Received CMSG_BUG [Bug Report]");
    else
        NC_LOG_DEBUG("network", "WORLD: Received CMSG_BUG [Suggestion]");

    NC_LOG_DEBUG("network", "%s", type.c_str());
    NC_LOG_DEBUG("network", "%s", content.c_str());
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_RECLAIM_CORPSE");
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_RESURRECT_RESPONSE");
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "CMSG_AREATRIGGER. Trigger ID: %u", triggerId);

    Player* player = GetPlayer();
    if (player->IsInFlight())
    {
        NC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, ignore Area Trigger ID:%u",
    ...
    if (!atEntry)
    {
        NC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID:%u",
            player->GetName().c_str(), player->GetGUIDLow(), triggerId);
        return;
    }
    ...
    {
        NC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (trigger map: %u player map: %u), ignore Area Trigger ID: %u",
    ...
    if (atEntry->radius > 0)
    {
        ...
        {
            NC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (radius: %f distance: %f), ignore Area Trigger ID: %u",
    ...
    else
    {
        ...
        {
            NC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (1/2 box X: %f 1/2 box Y: %f 1/2 box Z: %f rotatedPlayerX: %f rotatedPlayerY: %f dZ:%f), ignore Area Trigger ID: %u",
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");
    ...
    if (decompressedSize > 0xFFFF)
    {
        ...
        NC_LOG_DEBUG("network", "UAD: Account data packet too big, size %u", decompressedSize);
        return;
    }
    ...
    {
        recvData.rfinish();                   // unneeded warning spam in this case
        NC_LOG_DEBUG("network", "UAD: Failed to decompress account data");
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type = recvData.ReadBits(3);

    NC_LOG_DEBUG("network", "RAD: type %u", type);
    ...
    {
        NC_LOG_DEBUG("network", "RAD: Failed to compress account data");
        return;
    }
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "CMSG_SET_ACTION_BUTTON slotId: %u actionId: %u", slotId, button->id);
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_COMPLETE_CINEMATIC");
}

void WorldSession::HandleNextCinematicCamera(WorldPacket& /*recvData*/)
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_NEXT_CINEMATIC_CAMERA");
}

void WorldSession::HandleMoveTimeSkippedOpcode(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_MOVE_TIME_SKIPPED");
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_FEATHER_FALL_ACK");
...
/*
    ...
    NC_LOG_DEBUG("network", "WORLD: CMSG_FORCE_MOVE_UNROOT_ACK");
...
/*
    ...
    NC_LOG_DEBUG("network", "WORLD: CMSG_FORCE_MOVE_ROOT_ACK");
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recvData)
{
    ...
    {
        if (actionBar != 0)
            NC_LOG_ERROR("network", "WorldSession::HandleSetActionBarToggles in not logged state with value: %u, ignored", uint32(actionBar));
        return;
    }
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_INSPECT");

    Player* player = ObjectAccessor::FindPlayer(guid);
    if (!player)
    {
        NC_LOG_DEBUG("network", "CMSG_INSPECT: No player found from GUID: " UI64FMTD, (uint64)guid);
        return;
    }
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        NC_LOG_DEBUG("network", "CMSG_INSPECT_HONOR_STATS: No player found from GUID: " UI64FMTD, (uint64)guid);
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_WORLD_TELEPORT");

    if (GetPlayer()->IsInFlight())
    {
        NC_LOG_DEBUG("network", "Player '%s' (GUID: %u) in flight, ignore worldport command.",
            GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
        return;
    }

    NC_LOG_DEBUG("network", "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y = %f, z = %f, o = %f",
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_WHOIS");
    std::string charname;
    recvData >> charname;
    ...
    NC_LOG_DEBUG("network", "Received whois command from player %s for character %s",
        GetPlayer()->GetName().c_str(), charname.c_str());
}

void WorldSession::HandleComplainOpcode(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_COMPLAIN");
    ...
    NC_LOG_DEBUG("network", "REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u, unk4 %u, message %s", spam_type, GUID_LOPART(spammer_guid), unk1, unk2, unk3, unk4, description.c_str());
}

void WorldSession::HandleRealmSplitOpcode(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "CMSG_REALM_SPLIT");
    ...
    SendPacket(&data);
    //NC_LOG_DEBUG("response sent %u", unk);
}

void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_FAR_SIGHT");
    ...
    if (apply)
    {
        NC_LOG_DEBUG("network", "Added FarSight " UI64FMTD " to player %u", _player->GetUInt64Value(PLAYER_FIELD_FARSIGHT_OBJECT), _player->GetGUIDLow());
        if (WorldObject* target = _player->GetViewpoint())
            _player->SetSeer(target);
        else
            NC_LOG_ERROR("network", "Player %s (GUID: %u) requests non-existing seer " UI64FMTD, _player->GetName().c_str(), GUID_LOPART(_player->GetGUID()), _player->GetUInt64Value(PLAYER_FIELD_FARSIGHT_OBJECT));
    }
    else
    {
        NC_LOG_DEBUG("network", "Player %u set vision to self", _player->GetGUIDLow());
...
{
    NC_LOG_DEBUG("network", "CMSG_SET_TITLE");
...
{
    NC_LOG_DEBUG("network", "CMSG_TIME_SYNC_RESPONSE");

    uint32 counter, clientTicks;
    recvData >> counter >> clientTicks;

    if (counter != _player->m_timeSyncQueue.front())
        NC_LOG_ERROR("network", "Wrong time sync counter from player %s (cheater?)", _player->GetName().c_str());

    NC_LOG_DEBUG("network", "Time sync received: counter %u, client ticks %u, time since last sync %u", counter, clientTicks, clientTicks - _player->m_timeSyncClient);

    uint32 ourTicks = clientTicks + (getMSTime() - _player->m_timeSyncServer);

    // diff should be small
    NC_LOG_DEBUG("network", "Our ticks: %u, diff %u, latency %u", ourTicks, ourTicks - clientTicks, GetLatency());
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_RESET_INSTANCES");
...
{
    NC_LOG_DEBUG("network", "Received: CMSG_SET_DUNGEON_DIFFICULTY");
    ...
    if (!difficultyEntry)
    {
        NC_LOG_DEBUG("network", "%d sent an invalid instance mode %u!",
    ...
    {
        NC_LOG_DEBUG("network", "%d sent an non-dungeon instance mode %d!",
    ...
    {
        NC_LOG_DEBUG("network", "player (Name: %s, GUID: %u) tried to reset the instance while player is inside!",
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    NC_LOG_DEBUG("network", "player %d tried to reset the instance while group member (Name: %s, GUID: %u) is inside!",
...
{
    NC_LOG_DEBUG("network", "Received: CMSG_SET_RAID_DIFFICULTY");
    ...
    if (map && map->IsRaid())
    {
        NC_LOG_DEBUG("network", "player %d tried to reset the raid while inside!", _player->GetGUIDLow());
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_CANCEL_MOUNT_AURA");
...
{
    // fly mode on/off
    NC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_SET_CAN_FLY_ACK");
...
{
    /*
        NC_LOG_DEBUG("network", "WORLD: CMSG_REQUEST_PET_INFO");
        recvData.hexlike();
    */
}
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_SET_TAXI_BENCHMARK_MODE");
    ...
    NC_LOG_DEBUG("network", "Client used \"/timetest %d\" command", mode);
}
...
void WorldSession::HandleQueryInspectAchievements(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        NC_LOG_DEBUG("network", "CMSG_QUERY_INSPECT_ACHIEVEMENTS: Inspected Player " UI64FMTD, (uint64)guid);
        return;
    }
...
{
    // empty opcode
    NC_LOG_DEBUG("network", "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
...
{
    // empty opcode
    NC_LOG_DEBUG("network", "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUEUE");
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket)
{
    ...
    {
        NC_LOG_INFO("network", "InstanceLockResponse: Player %s (guid %u) tried to bind himself/teleport to graveyard without a pending bind!",
            _player->GetName().c_str(), _player->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleRequestHotfix(WorldPacket& recvPacket)
{
    ...
    if (!store)
    {
        NC_LOG_ERROR("network", "CMSG_REQUEST_HOTFIX: Received unknown hotfix type: %u", type);
        recvPacket.rfinish();
        return;
    }
    ...
    for (uint32 i = 0; i < count; ++i)
    {
        ...
        NC_LOG_DEBUG("network", "SMSG_DB_REPLY: Sent hotfix entry: %u type: %u", entry, type);
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_UPDATE_MISSILE_TRAJECTORY");
...
void WorldSession::HandleObjectUpdateFailedOpcode(WorldPacket& recvPacket)
{
    ...
    NC_LOG_ERROR("network", "Object update failed for object " UI64FMTD " (%s) for player %s (%u)", uint64(guid), obj ? obj->GetName().c_str() : "object-not-found", GetPlayerName().c_str(), GetGuidLow());
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_SAVE_CUF_PROFILES");

    uint8 count = (uint8)recvPacket.ReadBits(20);

    if (count > MAX_CUF_PROFILES)
    {
        NC_LOG_ERROR("entities.player", "HandleSaveCUFProfiles - %s tried to save more than %i CUF profiles. Hacking attempt?", GetPlayerName().c_str(), MAX_CUF_PROFILES);
        recvPacket.rfinish();
        return;
    }
