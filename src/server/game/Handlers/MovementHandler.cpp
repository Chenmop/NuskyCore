...
{
    NC_LOG_DEBUG("network", "WORLD: got MSG_MOVE_WORLDPORT_ACK.");
...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    {
        NC_LOG_ERROR("network", "Player %s (GUID: %u) is still in world when teleported from map %s (%u) to new map %s (%u)", GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()), oldMap->GetMapName(), oldMap->GetId(), newMap ? newMap->GetMapName() : "Unknown", loc.GetMapId());
    ...
    {
        NC_LOG_ERROR("network", "Map %d (%s) could not be created for player %d (%s), porting player to homebind", loc.GetMapId(), newMap ? newMap->GetMapName() : "Unknown", GetPlayer()->GetGUIDLow(), GetPlayer()->GetName().c_str());
    ...
    {
        NC_LOG_ERROR("network", "WORLD: failed to teleport player %s (%d) to map %d (%s) because of unknown reason!",
...
{
    NC_LOG_DEBUG("network", "CMSG_MOVE_TELEPORT_ACK");
    ...
    NC_LOG_DEBUG("network", "Guid " UI64FMTD, uint64(guid));
    NC_LOG_DEBUG("network", "Flags %u, time %u", flags, time/IN_MILLISECONDS);
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvPacket)
{
    ...
    {
        NC_LOG_ERROR("network", "HandleMovementOpcodes: guid error");
        return;
    }
    if (!movementInfo.pos.IsPositionValid())
    {
        NC_LOG_ERROR("network", "HandleMovementOpcodes: Invalid Position");
        return;
    }
    ...
    if (movementInfo.transport.guid)
    {
        ...
        if (!Nusky::IsValidMapCoord(movementInfo.pos.GetPositionX() + movementInfo.transport.pos.GetPositionX(), movementInfo.pos.GetPositionY() + movementInfo.transport.pos.GetPositionY(),
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket &recvData)
{
    ...
    switch (opcode)
    {
        ...
        default:
            NC_LOG_ERROR("network", "WorldSession::HandleForceSpeedChangeAck: Unknown move type opcode: %u", opcode);
            return;
    }
    ...
    if (!_player->GetTransport() && fabs(_player->GetSpeed(move_type) - newspeed) > 0.01f)
    {
        ...
        {
            NC_LOG_ERROR("network", "%sSpeedChange player %s is NOT correct (must be %f instead %f), force set to correct value",
                move_type_name[move_type], _player->GetName().c_str(), _player->GetSpeed(move_type), newspeed);
            _player->SetSpeed(move_type, _player->GetSpeedRate(move_type), true);
        }
        else                                                // must be lesser - cheating
        {
            NC_LOG_DEBUG("misc", "Player %s from account id %u kicked for incorrect speed (must be %f instead %f)",
...
{
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_SET_ACTIVE_MOVER");
    ...
    if (GetPlayer()->IsInWorld())
    {
        if (_player->m_mover->GetGUID() != guid)
            NC_LOG_ERROR("network", "HandleSetActiveMoverOpcode: incorrect mover guid: mover is " UI64FMTD " (%s - Entry: %u) and should be " UI64FMTD, uint64(guid), GetLogNameForGuid(guid), GUID_ENPART(guid), _player->m_mover->GetGUID());
    }
}

void WorldSession::HandleMoveNotActiveMover(WorldPacket &recvData)
{
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_MOVE_NOT_ACTIVE_MOVER");
...
{
    NC_LOG_DEBUG("network", "CMSG_MOVE_KNOCK_BACK_ACK");
...
{
    NC_LOG_DEBUG("network", "CMSG_MOVE_HOVER_ACK");
...
{
    NC_LOG_DEBUG("network", "CMSG_MOVE_WATER_WALK_ACK");
...
{
    NC_LOG_DEBUG("network", "CMSG_MOVE_SET_COLLISION_HEIGHT_ACK");
