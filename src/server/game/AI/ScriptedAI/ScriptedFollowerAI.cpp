...
void FollowerAI::EnterEvadeMode()
{
    ...
    {
        NC_LOG_DEBUG("scripts", "FollowerAI left combat, returning to CombatStartPosition.");
...
void FollowerAI::UpdateAI(uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->GetVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            {
                NC_LOG_DEBUG("scripts", "FollowerAI is set completed, despawns.");
            ...
            if (Player* player = GetLeaderForFollower())
            {
                ...
                {
                    NC_LOG_DEBUG("scripts", "FollowerAI is returning to leader.");
            ...
            {
                NC_LOG_DEBUG("scripts", "FollowerAI failed because player/group was to far away or not found");
                me->DespawnOrUnsummon();
                return;
            }
...
void FollowerAI::StartFollow(Player* player, uint32 factionForFollower, const Quest* quest)
{
    if (me->GetVictim())
    {
        NC_LOG_DEBUG("scripts", "FollowerAI attempt to StartFollow while in combat.");
        return;
    }

    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        NC_LOG_ERROR("scripts", "FollowerAI attempt to StartFollow while already following.");
        return;
    }
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        NC_LOG_DEBUG("scripts", "FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
    }
    ...
    NC_LOG_DEBUG("scripts", "FollowerAI start follow %s (GUID " UI64FMTD ")", player->GetName().c_str(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower()
{
        ...
        else
        {
            if (Group* group = player->GetGroup())
            {
                for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
                {
                    ...
                    {
                        NC_LOG_DEBUG("scripts", "FollowerAI GetLeader changed and returned new leader.");
    ...
    NC_LOG_DEBUG("scripts", "FollowerAI GetLeader can not find suitable leader.");
    return NULL;
}
