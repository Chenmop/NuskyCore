...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        NC_LOG_ERROR("scripts", "Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", soundId, source->GetTypeId(), source->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        NC_LOG_ERROR("scripts", "DoResetThreat called for creature that either cannot have threat list or has empty threat list (me entry = %d)", me->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        NC_LOG_ERROR("scripts", "Creature " UI64FMTD " (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: " UI64FMTD ") to x: %f y:%f z: %f o: %f. Aborted.",
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float range)
{
    ...
    Nusky::CreatureListSearcher<Nusky::FriendlyCCedInRange> searcher(me, list, u_check);
    me->VisitNearbyObject(range, searcher);
...
{
    std::list<Creature*> list;
    Nusky::FriendlyMissingBuffInRange u_check(me, range, uiSpellid);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float minimumRange)
{
    ...
    CellCoord pair(Nusky::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    Nusky::PlayerAtMinimumRangeAway check(me, minimumRange);
    Nusky::PlayerSearcher<Nusky::PlayerAtMinimumRangeAway> searcher(me, player, check);
    TypeContainerVisitor<Nusky::PlayerSearcher<Nusky::PlayerAtMinimumRangeAway>, GridTypeMapContainer> visitor(searcher);
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(uint32 const diff)
{
    ...
    switch (me->GetEntry())
    {
        ...
        default: // For most of creatures that certain area is their home area.
            NC_LOG_INFO("misc", "TSCR: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition. Using the default one.", me->GetEntry());
