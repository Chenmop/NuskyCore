...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    if (!set)
    {
        NC_LOG_ERROR("sql.sql", "Item set %u for item (id %u) not found, mods not applied.", setid, proto->ItemId);
        return;
    }
    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x)
    {
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y)
        {
            if (!eff->spells[y])                             // free slot
            {
                ...
                if (!spellInfo)
                {
                    NC_LOG_ERROR("entities.player.items", "WORLD: unknown spell id %u in items set %u effects", set->spells[x], setid);
                    break;
                }
...
void RemoveItemsSetItem(Player*player, ItemTemplate const* proto)
{
    ...
    if (!set)
    {
        NC_LOG_ERROR("sql.sql", "Item set #%u for item #%u not found, mods not removed.", setid, proto->ItemId);
        return;
    }
...
void Item::UpdateDuration(Player* owner, uint32 diff)
{
    ...
    NC_LOG_DEBUG("entities.player.items", "Item::UpdateDuration Item (Entry: %u Duration %u Diff %u)", GetEntry(), GetUInt32Value(ITEM_FIELD_EXPIRATION), diff);
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id)
{
    ...
    if ((itemProto->RandomProperty) && (itemProto->RandomSuffix))
    {
        NC_LOG_ERROR("sql.sql", "Item template %u have RandomProperty == %u and RandomSuffix == %u, but must have one from field =0", itemProto->ItemId, itemProto->RandomProperty, itemProto->RandomSuffix);
        return 0;
    }
    ...
    if (itemProto->RandomProperty)
    {
        ...
        if (!random_id)
        {
            NC_LOG_ERROR("sql.sql", "Enchantment id #%u used but it doesn't have records in 'ItemRandomProperties.dbc'", randomPropId);
            return 0;
        }
    ...
    else
    {
        ...
        if (!random_id)
        {
            NC_LOG_ERROR("sql.sql", "Enchantment id #%u used but it doesn't have records in sItemRandomSuffixStore.", randomPropId);
            return 0;
        }
...
void Item::AddToUpdateQueueOf(Player* player)
{
    ...
    {
        NC_LOG_DEBUG("entities.player.items", "Item::AddToUpdateQueueOf - Owner's guid (%u) and player's guid (%u) don't match!", GUID_LOPART(GetOwnerGUID()), player->GetGUIDLow());
        return;
    }
...
void Item::RemoveFromUpdateQueueOf(Player* player)
{
    ...
    if (player->GetGUID() != GetOwnerGUID())
    {
        NC_LOG_DEBUG("entities.player.items", "Item::RemoveFromUpdateQueueOf - Owner's guid (%u) and player's guid (%u) don't match!", GUID_LOPART(GetOwnerGUID()), player->GetGUIDLow());
        return;
    }
