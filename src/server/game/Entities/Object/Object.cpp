...
WorldObject::~WorldObject()
{
    ...
    if (IsWorldObject() && m_currMap)
    {
        ...
        {
            NC_LOG_FATAL("misc", "Object::~Object Corpse guid=" UI64FMTD ", type=%d, entry=%u deleted but still in map!!",
...
Object::~Object()
{
    if (IsInWorld())
    {
        NC_LOG_FATAL("misc", "Object::~Object - guid=" UI64FMTD ", typeid=%d, entry=%u deleted but still in world!!", GetGUID(), GetTypeId(), GetEntry());
        if (isType(TYPEMASK_ITEM))
            NC_LOG_FATAL("misc", "Item slot %u", ((Item*)this)->GetSlot());
        ASSERT(false);
        RemoveFromWorld();
    }
    ...
    if (m_objectUpdated)
    {
        NC_LOG_FATAL("misc", "Object::~Object - guid=" UI64FMTD ", typeid=%d, entry=%u deleted but still in update list!!", GetGUID(), GetTypeId(), GetEntry());
        ASSERT(false);
...
void Object::SetByteValue(uint16 index, uint8 offset, uint8 value)
{
    ...
    if (offset > 4)
    {
        NC_LOG_ERROR("misc", "Object::SetByteValue: wrong offset %u", offset);
        return;
    }
...
void Object::SetUInt16Value(uint16 index, uint8 offset, uint16 value)
{
    ...
    if (offset > 2)
    {
        NC_LOG_ERROR("misc", "Object::SetUInt16Value: wrong offset %u", offset);
        return;
    }
...
void Object::SetByteFlag(uint16 index, uint8 offset, uint8 newFlag)
{
    ...
    if (offset > 4)
    {
        NC_LOG_ERROR("misc", "Object::SetByteFlag: wrong offset %u", offset);
        return;
    }
...
void Object::RemoveByteFlag(uint16 index, uint8 offset, uint8 oldFlag)
{
    ...
    if (offset > 4)
    {
        NC_LOG_ERROR("misc", "Object::RemoveByteFlag: wrong offset %u", offset);
        return;
    }
...
{
    NC_LOG_ERROR("misc", "Attempt %s non-existed value field: %u (count: %u) for object typeid: %u type mask: %u", (set ? "set value to" : "get value from"), index, m_valuesCount, GetTypeId(), m_objectType);
...
{
    NC_LOG_INFO("misc", "MOVEMENT INFO");
    NC_LOG_INFO("misc", "guid " UI64FMTD, guid);
    NC_LOG_INFO("misc", "flags %s (%u)", Movement::MovementFlags_ToString(flags).c_str(), flags);
    NC_LOG_INFO("misc", "flags2 %s (%u)", Movement::MovementFlagsExtra_ToString(flags2).c_str(), flags2);
    NC_LOG_INFO("misc", "time %u current time %u", time, getMSTime());
    NC_LOG_INFO("misc", "position: `%s`", pos.ToString().c_str());
    if (transport.guid)
    {
        NC_LOG_INFO("misc", "TRANSPORT:");
        NC_LOG_INFO("misc", "guid: " UI64FMTD, transport.guid);
        NC_LOG_INFO("misc", "position: `%s`", transport.pos.ToString().c_str());
        NC_LOG_INFO("misc", "seat: %i", transport.seat);
        NC_LOG_INFO("misc", "time: %u", transport.time);
        if (flags2 & MOVEMENTFLAG2_INTERPOLATED_MOVEMENT)
            NC_LOG_INFO("misc", "time2: %u", transport.time2);
        if (transport.time3)
            NC_LOG_INFO("misc", "time3: %u", transport.time3);
    }

    if ((flags & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) || (flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING))
        NC_LOG_INFO("misc", "pitch: %f", pitch);

    if (flags & MOVEMENTFLAG_FALLING || jump.fallTime)
    {
        NC_LOG_INFO("misc", "fallTime: %u j_zspeed: %f", jump.fallTime, jump.zspeed);
        if (flags & MOVEMENTFLAG_FALLING)
            NC_LOG_INFO("misc", "j_sinAngle: %f j_cosAngle: %f j_xyspeed: %f", jump.sinAngle, jump.cosAngle, jump.xyspeed);
    }

    if (flags & MOVEMENTFLAG_SPLINE_ELEVATION)
        NC_LOG_INFO("misc", "splineElevation: %f", splineElevation);
}
...
void WorldObject::GetRandomPoint(const Position &pos, float distance, float &rand_x, float &rand_y, float &rand_z) const
{
    ...
    Nusky::NormalizeMapCoord(rand_x);
    Nusky::NormalizeMapCoord(rand_y);
...
{
    return Nusky::IsValidMapCoord(m_positionX, m_positionY, m_positionZ, m_orientation);
}
...
namespace Nusky
{
    class MonsterChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetNuskyString(i_textId, loc_idx);
...
void WorldObject::MonsterSay(const char* text, uint32 language, WorldObject const* target)
{
    ...
    Nusky::MonsterCustomChatBuilder say_build(this, CHAT_MSG_MONSTER_SAY, text, language, target);
    Nusky::LocalizedPacketDo<Nusky::MonsterCustomChatBuilder> say_do(say_build);
    Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterCustomChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
    TypeContainerVisitor<Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterCustomChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void WorldObject::MonsterSay(int32 textId, uint32 language, WorldObject const* target)
{
    ...
    Nusky::MonsterChatBuilder say_build(this, CHAT_MSG_MONSTER_SAY, textId, language, target);
    Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> say_do(say_build);
    Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
    TypeContainerVisitor<Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
    cell.Visit(p, message, *GetMap(), *this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY));
}
...
void WorldObject::MonsterYell(const char* text, uint32 language, WorldObject const* target)
{
    ...
    Nusky::MonsterCustomChatBuilder say_build(this, CHAT_MSG_MONSTER_YELL, text, language, target);
    Nusky::LocalizedPacketDo<Nusky::MonsterCustomChatBuilder> say_do(say_build);
    Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterCustomChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_YELL), say_do);
    TypeContainerVisitor<Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterCustomChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void WorldObject::MonsterYell(int32 textId, uint32 language, WorldObject const* target)
{
    ...
    Nusky::MonsterChatBuilder say_build(this, CHAT_MSG_MONSTER_YELL, textId, language, target);
    Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> say_do(say_build);
    Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_YELL), say_do);
    TypeContainerVisitor<Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
    cell.Visit(p, message, *GetMap(), *this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_YELL));
}
...
void WorldObject::MonsterTextEmote(int32 textId, WorldObject const* target, bool IsBossEmote)
{
    ...
    Nusky::MonsterChatBuilder say_build(this, IsBossEmote ? CHAT_MSG_RAID_BOSS_EMOTE : CHAT_MSG_MONSTER_EMOTE, textId, LANG_UNIVERSAL, target);
    Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> say_do(say_build);
    Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), say_do);
    TypeContainerVisitor<Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void WorldObject::MonsterWhisper(int32 textId, Player const* target, bool IsBossWhisper)
{
    ...
    LocaleConstant loc_idx = target->GetSession()->GetSessionDbLocaleIndex();
    char const* text = sObjectMgr->GetNuskyString(textId, loc_idx);
...
{
    Nusky::MessageDistDeliverer notifier(this, data, dist);
    VisitNearbyWorldObject(dist, notifier);
}

void WorldObject::SendMessageToSet(WorldPacket* data, Player const* skipped_rcvr)
{
    Nusky::MessageDistDeliverer notifier(this, data, GetVisibilityRange(), false, skipped_rcvr);
...
void WorldObject::SetMap(Map* map)
{
    ...
    if (m_currMap)
    {
        NC_LOG_FATAL("misc", "WorldObject::SetMap: obj %u new map %u %u, old map %u %u", (uint32)GetTypeId(), map->GetId(), map->GetInstanceId(), m_currMap->GetId(), m_currMap->GetInstanceId());
        ASSERT(false);
    }
    m_currMap = map;
    m_mapId = map->GetId();
...
void WorldObject::AddObjectToRemoveList()
{
    ...
    if (!map)
    {
        NC_LOG_ERROR("misc", "Object (TypeId: %u Entry: %u GUID: %u) at attempt add to move list not have valid map (Id: %u).", GetTypeId(), GetEntry(), GetGUIDLow(), GetMapId());
        return;
    }
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime)
{
    ...
    if (!goinfo)
    {
        NC_LOG_ERROR("sql.sql", "Gameobject template %u not found in database!", entry);
        return NULL;
    }
...
Creature* WorldObject::FindNearestCreature(uint32 entry, float range, bool alive) const
{
    ...
    Nusky::NearestCreatureEntryWithLiveStateInObjectRangeCheck checker(*this, entry, alive, range);
    Nusky::CreatureLastSearcher<Nusky::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(this, creature, checker);
...
{
    GameObject* go = NULL;
    Nusky::NearestGameObjectEntryInObjectRangeCheck checker(*this, entry, range);
    Nusky::GameObjectLastSearcher<Nusky::NearestGameObjectEntryInObjectRangeCheck> searcher(this, go, checker);
...
{
    GameObject* go = NULL;
    Nusky::NearestGameObjectTypeInObjectRangeCheck checker(*this, type, range);
    Nusky::GameObjectLastSearcher<Nusky::NearestGameObjectTypeInObjectRangeCheck> searcher(this, go, checker);
    VisitNearbyGridObject(range, searcher);
    return go;
}
...
{
    CellCoord pair(Nusky::ComputeCellCoord(this->GetPositionX(), this->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    Nusky::AllGameObjectsWithEntryInRange check(this, entry, maxSearchRange);
    Nusky::GameObjectListSearcher<Nusky::AllGameObjectsWithEntryInRange> searcher(this, gameobjectList, check);
    TypeContainerVisitor<Nusky::GameObjectListSearcher<Nusky::AllGameObjectsWithEntryInRange>, GridTypeMapContainer> visitor(searcher);
...
void WorldObject::GetCreatureListWithEntryInGrid(std::list<Creature*>& creatureList, uint32 entry, float maxSearchRange) const
{
    ...
    Nusky::AllCreaturesOfEntryInRange check(this, entry, maxSearchRange);
    Nusky::CreatureListSearcher<Nusky::AllCreaturesOfEntryInRange> searcher(this, creatureList, check);
    TypeContainerVisitor<Nusky::CreatureListSearcher<Nusky::AllCreaturesOfEntryInRange>, GridTypeMapContainer> visitor(searcher);
...
{
    Nusky::AnyPlayerInObjectRangeCheck checker(this, maxSearchRange);
    Nusky::PlayerListSearcher<Nusky::AnyPlayerInObjectRangeCheck> searcher(this, playerList, checker);
...
void WorldObject::GetNearPoint2D(float &x, float &y, float distance2d, float absAngle) const
{
    ...
    Nusky::NormalizeMapCoord(x);
    Nusky::NormalizeMapCoord(y);
}
...
void WorldObject::GetNearPoint(WorldObject const* /*searcher*/, float &x, float &y, float &z, float searcher_size, float distance2d, float absAngle) const
{
    ...
    // adding used positions around object
    {
        ...
        Nusky::NearUsedPosDo u_do(*this, searcher, absAngle, selector);
        Nusky::WorldObjectWorker<Nusky::NearUsedPosDo> worker(this, u_do);

        TypeContainerVisitor<Nusky::WorldObjectWorker<Nusky::NearUsedPosDo>, GridTypeMapContainer  > grid_obj_worker(worker);
        TypeContainerVisitor<Nusky::WorldObjectWorker<Nusky::NearUsedPosDo>, WorldTypeMapContainer > world_obj_worker(worker);
...
void WorldObject::MovePosition(Position &pos, float dist, float angle)
{
    ...
    if (!Nusky::IsValidMapCoord(destx, desty, pos.m_positionZ))
    {
        NC_LOG_FATAL("misc", "WorldObject::MovePosition: Object (TypeId: %u Entry: %u GUID: %u) has invalid coordinates X: %f and Y: %f were passed!",
    ...
    Nusky::NormalizeMapCoord(pos.m_positionX);
    Nusky::NormalizeMapCoord(pos.m_positionY);
...
void WorldObject::MovePositionToFirstCollision(Position &pos, float dist, float angle)
{
    ...
    // Prevent invalid coordinates here, position is unchanged
    if (!Nusky::IsValidMapCoord(destx, desty))
    {
        NC_LOG_FATAL("misc", "WorldObject::MovePositionToFirstCollision invalid coordinates X: %f and Y: %f were passed!", destx, desty);
        return;
    }
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    Nusky::AnyPlayerInObjectRangeCheck check(this, GetVisibilityRange(), false);
    Nusky::PlayerListSearcher<Nusky::AnyPlayerInObjectRangeCheck> searcher(this, targets, check);
...
void WorldObject::UpdateObjectVisibility(bool /*forced*/)
{
    ...
    Nusky::VisibleChangesNotifier notifier(*this);
    VisitNearbyWorldObject(GetVisibilityRange(), notifier);
}
