...
Creature::~Creature()
{
    ...
    //if (m_uint32Values)
    //    NC_LOG_ERROR("entities.unit", "Deconstruct Creature Entry = %u", GetEntry());
}
...
bool Creature::InitEntry(uint32 entry, uint32 /*team*/, const CreatureData* data)
{
    ...
    if (!normalInfo)
    {
        NC_LOG_ERROR("sql.sql", "Creature::InitEntry creature entry %u does not exist.", entry);
        return false;
    }
    ...
    {
        NC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has no model defined in table `creature_template`, can't load. ", entry);
        return false;
    }
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        case JUST_RESPAWNED:
            ...
            NC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_RESPAWNED (4)", GetGUIDLow(), GetEntry());
            break;
        case JUST_DIED:
            // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE promoting.
            NC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_DEAD (1)", GetGUIDLow(), GetEntry());
            break;
        case DEAD:
        {
        ...
        case CORPSE:
        {
            ...
            {
                RemoveCorpse(false);
                NC_LOG_DEBUG("entities.unit", "Removing corpse... %u ", GetUInt32Value(OBJECT_FIELD_ENTRY_ID));
            }
            break;
        }
        case ALIVE:
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius >0)
    {
        ...
        Cell cell(p);
        cell.SetNoCreate();
        Nusky::NearestAssistCreatureInCreatureRangeCheck u_check(this, GetVictim(), radius);
        Nusky::CreatureLastSearcher<Nusky::NearestAssistCreatureInCreatureRangeCheck> searcher(this, creature, u_check);

        TypeContainerVisitor<Nusky::CreatureLastSearcher<Nusky::NearestAssistCreatureInCreatureRangeCheck>, GridTypeMapContainer > grid_creature_searcher(searcher);
...
bool Creature::AIM_Initialize(CreatureAI* ai)
{
    ...
    if (m_AI_locked)
    {
        NC_LOG_DEBUG("scripts", "AIM_Initialize: failed to init, locked.");
        return false;
    }
...
bool Creature::Create(uint32 guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, uint32 team, float x, float y, float z, float ang, const CreatureData* data)
{
    ...
    if (!cinfo)
    {
        NC_LOG_ERROR("sql.sql", "Creature::Create(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, Entry);
        return false;
    }
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("entities.unit", "Creature::Create(): given coordinates for creature (guidlow %d, entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)", guidlow, Entry, x, y, z, ang);
        return false;
    }
...
void Creature::SaveToDB()
{
    ...
    if (!data)
    {
        NC_LOG_ERROR("entities.unit", "Creature::SaveToDB failed, cannot get creature data!");
        return;
    }
...
bool Creature::CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId, uint32 team, const CreatureData* data)
{
    ...
    if (!cinfo)
    {
        NC_LOG_ERROR("sql.sql", "Creature::CreateFromProto(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, Entry);
        return false;
    }
...
bool Creature::LoadCreatureFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        NC_LOG_ERROR("sql.sql", "Creature (GUID: %u) not found in table `creature`, can't load. ", guid);
        return false;
    }
...
void Creature::DeleteFromDB()
{
    if (!m_DBTableGuid)
    {
        NC_LOG_ERROR("entities.unit", "Trying to delete not saved creature! LowGUID: %u, Entry: %u", GetGUIDLow(), GetEntry());
        return;
    }
...
void Creature::Respawn(bool force)
{
    ...
    if (getDeathState() == DEAD)
    {
        ...
        NC_LOG_DEBUG("entities.unit", "Respawning creature %s (GuidLow: %u, Full GUID: " UI64FMTD " Entry: %u)",
...
SpellInfo const* Creature::reachWithSpellAttack(Unit* victim)
{
    ...
    for (uint32 i=0; i < CREATURE_MAX_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            NC_LOG_ERROR("entities.unit", "WORLD: unknown spell id %i", m_spells[i]);
            continue;
        }
...
Unit* Creature::SelectNearestTarget(float dist, bool playerOnly /* = false */) const
{
...

    {
        ...
        Nusky::NearestHostileUnitCheck u_check(this, dist, playerOnly);
        Nusky::UnitLastSearcher<Nusky::NearestHostileUnitCheck> searcher(this, target, u_check);

        TypeContainerVisitor<Nusky::UnitLastSearcher<Nusky::NearestHostileUnitCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        TypeContainerVisitor<Nusky::UnitLastSearcher<Nusky::NearestHostileUnitCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    {
        NC_LOG_ERROR("entities.unit", "Creature (GUID: %u Entry: %u) SelectNearestTargetInAttackDistance called with dist > MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.", GetGUIDLow(), GetEntry());
        dist = ATTACK_DISTANCE;
    }

    {
        Nusky::NearestHostileUnitInAttackDistanceCheck u_check(this, dist);
        Nusky::UnitLastSearcher<Nusky::NearestHostileUnitInAttackDistanceCheck> searcher(this, target, u_check);

        TypeContainerVisitor<Nusky::UnitLastSearcher<Nusky::NearestHostileUnitInAttackDistanceCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        TypeContainerVisitor<Nusky::UnitLastSearcher<Nusky::NearestHostileUnitInAttackDistanceCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
Player* Creature::SelectNearestPlayer(float distance) const
{
    ...
    Nusky::NearestPlayerInObjectRangeCheck checker(this, distance);
    Nusky::PlayerLastSearcher<Nusky::NearestPlayerInObjectRangeCheck> searcher(this, target, checker);
...
void Creature::SendAIReaction(AiReaction reactionType)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_AI_REACTION, type %u.", reactionType);
}
    ...
    if (!m_AlreadyCallAssistance && GetVictim() && !IsPet() && !IsCharmed())
    {
        ...
        if (radius > 0)
        {
...

            {
                ...
                Nusky::AnyAssistCreatureInRangeCheck u_check(this, GetVictim(), radius);
                Nusky::CreatureListSearcher<Nusky::AnyAssistCreatureInRangeCheck> searcher(this, assistList, u_check);

                TypeContainerVisitor<Nusky::CreatureListSearcher<Nusky::AnyAssistCreatureInRangeCheck>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void Creature::CallForHelp(float radius)
{
    ...
    Nusky::CallOfHelpCreatureInRangeDo u_do(this, GetVictim(), radius);
    Nusky::CreatureWorker<Nusky::CallOfHelpCreatureInRangeDo> worker(this, u_do);

    TypeContainerVisitor<Nusky::CreatureWorker<Nusky::CallOfHelpCreatureInRangeDo>, GridTypeMapContainer >  grid_creature_searcher(worker);
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    ...
    if (!cainfo->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
        {
            ...
            {
                NC_LOG_ERROR("sql.sql", "Creature (GUID: %u Entry: %u) has wrong spell %u defined in `auras` field.", GetGUIDLow(), GetEntry(), *itr);
                continue;
            }
            ...
            {
                if (!reload)
                    NC_LOG_ERROR("sql.sql", "Creature (GUID: %u Entry: %u) has duplicate aura (spell %u) in `auras` field.", GetDBTableGUIDLow(), GetEntry(), *itr);

                continue;
            }

            AddAura(*itr, this);
            NC_LOG_DEBUG("entities.unit", "Spell: %u added to creature (GUID: %u Entry: %u)", *itr, GetGUIDLow(), GetEntry());
        }
    }
...
void Creature::SetInCombatWithZone()
{
    ...
    {
        NC_LOG_ERROR("entities.unit", "Creature entry %u call SetInCombatWithZone but creature cannot have threat list.", GetEntry());
        return;
    }
    ...
    if (!map->IsInstance())
    {
        NC_LOG_ERROR("entities.unit", "Creature entry %u call SetInCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(), map->GetId());
        return;
    }
...
void Creature::SetPosition(float x, float y, float z, float o)
{
    ...
    if (!Nusky::IsValidMapCoord(x, y, z, o))
    {
        NC_LOG_DEBUG("entities.unit", "Creature::SetPosition(%f, %f, %f) .. bad coordinates!", x, y, z);
        return;
    }
...
Unit* Creature::SelectNearestHostileUnitInAggroRange(bool useLOS) const
{
    ...
    {
        Nusky::NearestHostileUnitInAggroRangeCheck u_check(this, useLOS);
        Nusky::UnitSearcher<Nusky::NearestHostileUnitInAggroRangeCheck> searcher(this, target, u_check);
