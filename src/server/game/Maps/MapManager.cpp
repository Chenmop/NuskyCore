...
void MapManager::checkAndCorrectGridStatesArray()
{
    ...
    for (int i=0; i<MAX_GRID_STATE; i++)
    {
        ...
        {
            NC_LOG_ERROR("maps", "MapManager::checkGridStates(), GridState: si_GridStates is currupt !!!");
            ok = false;
...
bool MapManager::CanPlayerEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    if (entry->IsRaid())
    {
        ...
        if ((!group || !group->isRaidGroup()) && !sWorld->getBoolConfig(CONFIG_INSTANCE_IGNORE_RAID))
        {
            ...
            player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetNuskyString(LANG_INSTANCE_RAID_GROUP_ONLY), mapName);
            NC_LOG_DEBUG("maps", "MAP: Player '%s' must be in a raid group to enter instance '%s'", player->GetName().c_str(), mapName);
            return false;
        }
    }
    ...
    if (!player->IsAlive())
    {
        if (Corpse* corpse = player->GetCorpse())
        {
            ...
            if (!corpseMap)
            {
                ...
                NC_LOG_DEBUG("maps", "MAP: Player '%s' does not have a corpse in instance '%s' and cannot enter.", player->GetName().c_str(), mapName);
                return false;
            }
            NC_LOG_DEBUG("maps", "MAP: Player '%s' has corpse in instance '%s' and can enter.", player->GetName().c_str(), mapName);
            player->ResurrectPlayer(0.5f, false);
            player->SpawnCorpseBones();
        }
        else
            NC_LOG_DEBUG("maps", "Map::CanPlayerEnter - player '%s' is dead but does not have a corpse!", player->GetName().c_str());
    }
...
uint32 MapManager::GenerateInstanceId()
{
    ...
    {
        NC_LOG_ERROR("maps", "Instance ID overflow!! Can't continue, shutting down server. ");
