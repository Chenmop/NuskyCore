...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param)
{
    ...
    NC_LOG_DEBUG("guild", "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s", session->GetPlayerInfo().c_str(), type, errCode, param.c_str());
}
...
void Guild::SendSaveEmblemResult(WorldSession* session, GuildEmblemError errCode)
{
    ...
    NC_LOG_DEBUG("guild", "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u", session->GetPlayerInfo().c_str(), errCode);
}
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, SQLTransaction& trans, bool logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i)
    {
        ...
        if (logOnCreate)
            NC_LOG_ERROR("guild", "Guild %u has broken Tab %u for rank %u. Created default tab.", m_guildId, i, m_rankId);
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    {
        NC_LOG_ERROR("guild", "Invalid slot for item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    if (!proto)
    {
        NC_LOG_ERROR("guild", "Unknown item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    {
        NC_LOG_ERROR("guild", "Item (GUID %u, id: %u) not found in item_instance, deleting from guild bank!", itemGuid, itemEntry);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    if (session)
    {
        NC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [%s]: Tabid: %u, Text: %s", session->GetPlayerInfo().c_str(), m_tabId, m_text.c_str());
        session->SendPacket(&data);
    }
    else
    {
        NC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [Broadcast]: Tabid: %u, Text: %s", m_tabId, m_text.c_str());
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId)
    {
        NC_LOG_ERROR("guild", "Player (GUID: %u) has broken zone-data", GUID_LOPART(m_guid));
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1)
    {
        NC_LOG_ERROR("guild", "Player (GUID: %u) has a broken data in field `characters`.`level`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }

    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES)
    {
        NC_LOG_ERROR("guild", "Player (GUID: %u) has a broken data in field `characters`.`class`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }
    return true;
}
...
Item* Guild::BankMoveItemData::StoreItem(SQLTransaction& trans, Item* pItem)
{
    ...
    for (ItemPosCountVec::const_iterator itr = m_vec.begin(); itr != m_vec.end(); )
    {
        ...
        NC_LOG_DEBUG("guild", "GUILD STORAGE: StoreItem tab = %u, slot = %u, item = %u, count = %u",
...
{
    NC_LOG_DEBUG("guild", "GUILD STORAGE: CanStore() tab = %u, slot = %u, item = %u, count = %u",
        m_container, m_slotId, pItem->GetEntry(), pItem->GetCount());
...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    NC_LOG_DEBUG("guild", "GUILD: creating guild [%s] for leader %s (%u)",
    ...
    if (Member* member = GetMember(player->GetGUID()))
    {
        switch (dataid)
        {
            ...
            case GUILD_MEMBER_DATA_LEVEL:
                ...
                break;
            default:
                NC_LOG_ERROR("guild", "Guild::UpdateMemberData: Called with incorrect DATAID %u (value %u)", dataid, value);
                return;
        }
        //HandleRoster();
    }
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    if (session)
    {
        NC_LOG_DEBUG("guild", "SMSG_GUILD_ROSTER [%s]", session->GetPlayerInfo().c_str());
        session->SendPacket(&data);
    }
    else
    {
        NC_LOG_DEBUG("guild", "SMSG_GUILD_ROSTER [Broadcast]");
        BroadcastPacket(&data);
    }
}
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_QUERY_RESPONSE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendGuildRankInfo(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_RANKS [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::HandleSetBankTabInfo(WorldSession* session, uint8 tabId, std::string const& name, std::string const& icon)
{
    ...
    if (!tab)
    {
        NC_LOG_ERROR("guild", "Guild::HandleSetBankTabInfo: Player %s trying to change bank tab info from unexisting tab %d.",
    ...
    {
        NC_LOG_ERROR("guild", "Guild::HandleSetBankTabInfo: Player %s is trying to modify bank tab info but doesn't have permission!",
...
void Guild::HandleSetMemberNote(WorldSession* session, std::string const& note, uint64 guid, bool isPublic)
{
    ...
    else if (Member* member = GetMember(guid))
    {
        ...
        if (session)
        {
            NC_LOG_DEBUG("guild", "SMSG_GUILD_MEMBER_UPDATE_NOTE [%s]", session->GetPlayerInfo().c_str());
            session->SendPacket(&data);
        }
        else
        {
            NC_LOG_DEBUG("guild", "SMSG_GUILD_MEMBER_UPDATE_NOTE [Broadcast]");
            BroadcastPacket(&data);
        }
    }
}
...
void Guild::HandleSetRankInfo(WorldSession* session, uint8 rankId, std::string const& name, uint32 rights, uint32 moneyPerDay, const GuildBankRightsAndSlotsVec& rightsAndSlots)
{
    ...
    {
        NC_LOG_DEBUG("guild", "Changed RankName to '%s', rights to 0x%08X", name.c_str(), rights);
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    NC_LOG_DEBUG("guild", "Player %s invited %s to join his Guild", player->GetName().c_str(), name.c_str());
    ...
    NC_LOG_DEBUG("guild", "SMSG_GUILD_INVITE [%s]", pInvitee->GetName().c_str());
}
...
void Guild::HandleDisband(WorldSession* session)
{
    ...
    {
        Disband();
        NC_LOG_DEBUG("guild", "Guild Successfully Disbanded");
        delete this;
    }
}
...
void Guild::HandleGuildPartyRequest(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_PARTY_STATE_RESPONSE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT_LOG_QUERY_RESULT [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendNewsUpdate(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_NEWS_UPDATE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        if (GetLevel() >= 5 && tabId == GUILD_BANK_MAX_TABS)
            data << uint64(0);
        session->SendPacket(&data);
        NC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_LOG_QUERY_RESULT [%s] TabId: %u", session->GetPlayerInfo().c_str(), tabId);
    }
}
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_PERMISSIONS_QUERY_RESULTS [%s] Rank: %u", session->GetPlayerInfo().c_str(), rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    data << int64(amount);
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: %u", session->GetPlayerInfo().c_str(), amount);
}
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    */
    ...
    NC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT [%s] MOTD", session->GetPlayerInfo().c_str());
    ...
    if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->CanInsert())
        {
            ...
            if (BankEventLogEntry::IsMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    NC_LOG_ERROR("guild", "GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: %u) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                NC_LOG_ERROR("guild", "GuildBankEventLog ERROR: non-money event (LogGuid: %u, Guild: %u) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
void Guild::LoadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
        NC_LOG_ERROR("guild", "Invalid tab (tabId: %u) in guild bank, skipped.", tabId);
    else
...
bool Guild::LoadBankItemFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
    {
        NC_LOG_ERROR("guild", "Invalid tab for item (GUID: %u, id: #%u) in guild bank, skipped.",
...
bool Guild::Validate()
{
    ...
    {
        NC_LOG_ERROR("guild", "Guild %u has invalid number of ranks, creating new...", m_id);
        broken_ranks = true;
    }
    else
    {
        for (uint8 rankId = 0; rankId < ranks; ++rankId)
        {
            ...
            {
                NC_LOG_ERROR("guild", "Guild %u has broken rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
            else
            {
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc)
{
    ...
    _CreateRank(sObjectMgr->GetNuskyString(LANG_GUILD_MASTER,   loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetNuskyString(LANG_GUILD_OFFICER,  loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetNuskyString(LANG_GUILD_VETERAN,  loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetNuskyString(LANG_GUILD_MEMBER,   loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetNuskyString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
...
void Guild::_MoveItems(MoveItemData* pSrc, MoveItemData* pDest, uint32 splitedAmount)
{
    ...
    {
        NC_LOG_FATAL("guild", "Guild::SwapItems: Player %s(GUIDLow: %u) tried to move item %u from tab %u slot %u to tab %u slot %u, but item %u has a stack of zero!",
...
void Guild::_SendBankContentUpdate(uint8 tabId, SlotIds slots) const
{
    if (BankTab const* tab = GetBankTab(tabId))
    {
        ...
        NC_LOG_DEBUG("guild", "WORLD: Sent (SMSG_GUILD_BANK_LIST)");
    }
}
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, uint64 guid, const char* param1, const char* param2, const char* param3) const
{
    ...
    if (sLog->ShouldLog("guild", LOG_LEVEL_DEBUG))
        NC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT [Broadcast] Event: %s (%u)", _GetGuildEventString(guildEvent).c_str(), guildEvent);
}
...
void Guild::SendGuildRanksUpdate(uint64 setterGuid, uint64 targetGuid, uint32 rank)
{
    ...
    NC_LOG_DEBUG("network", "SMSG_GUILD_RANKS_UPDATE [Broadcast] Target: %u, Issuer: %u, RankId: %u",
...
void Guild::SendGuildReputationWeeklyCap(WorldSession* session, uint32 reputation) const
{
    ...
    data << uint32(cap);
    session->SendPacket(&data);
    NC_LOG_DEBUG("guild", "SMSG_GUILD_REPUTATION_WEEKLY_CAP [%s]: Left: %u",
...
void Guild::HandleNewsSetSticky(WorldSession* session, uint32 newsId, bool sticky)
{
    ...
    if (itr == logs->end())
    {
        NC_LOG_DEBUG("guild", "HandleNewsSetSticky: [%s] requested unknown newsId %u - Sticky: %u",
    ...
    NC_LOG_DEBUG("guild", "HandleNewsSetSticky: [%s] chenged newsId %u sticky to %u",
        session->GetPlayerInfo().c_str(), newsId, sticky);
...
void Guild::HandleSetBankTabNote(WorldSession* session, uint32 tabId, std::string note)
{
    ...
    if (!bankTab)
    {
        NC_LOG_ERROR("guild", "Guild::HandleSetBankTabNote: Player %s is trying to modify bank tab note for non existing tab %d.",
            session->GetPlayerInfo().c_str(), tabId);
        return;
    }
    ...
    if (!_HasRankRight(session->GetPlayer(), GR_RIGHT_MODIFY_BANK_TAB))
    {
        NC_LOG_ERROR("guild", "Guild::HandleSetBankTabNote: Player %s is trying to modify a bank tab note but doesn't have permission!",
            session->GetPlayerInfo().c_str());
        return;
    }
