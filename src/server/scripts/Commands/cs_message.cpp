...
public:
    ...
    static bool HandleNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string msg = handler->GetNuskyString(LANG_GLOBAL_NOTIFY);
        msg += args;
    ...
    static bool HandleGMNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetNuskyString(LANG_GM_NOTIFY);
        str += args;
    ...
    static bool HandleWhispersCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, handler->GetSession()->GetPlayer()->isAcceptWhispers() ?  handler->GetNuskyString(LANG_ON) : handler->GetNuskyString(LANG_OFF));
            return true;
        }
