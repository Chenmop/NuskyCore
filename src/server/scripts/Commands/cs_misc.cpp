...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        CellCoord cellCoord = Nusky::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
        ...
        GridCoord gridCoord = Nusky::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetNuskyString(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target == handler->GetSession()->GetPlayer() ? handler->GetNuskyString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->RemoveSpellCooldown(spellIid, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, target == handler->GetSession()->GetPlayer() ? handler->GetNuskyString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetNuskyString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetNuskyString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetNuskyString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetNuskyString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        NC_LOG_DEBUG("misc", handler->GetNuskyString(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        NC_LOG_DEBUG("misc", handler->GetNuskyString(LANG_ADDITEMSET), itemSetId);
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string userName          = handler->GetNuskyString(LANG_ERROR);
        uint32 accId                  = 0;
        uint32 lowguid                = GUID_LOPART(targetGuid);
        std::string eMail             = handler->GetNuskyString(LANG_ERROR);
        std::string regMail           = handler->GetNuskyString(LANG_ERROR);
        uint32 security               = 0;
        std::string lastIp            = handler->GetNuskyString(LANG_ERROR);
        uint8 locked                  = 0;
        std::string lastLogin         = handler->GetNuskyString(LANG_ERROR);
        ...
        std::string genderStr           = handler->GetNuskyString(LANG_ERROR);
        uint32 totalPlayerTime          = 0;
        uint8 level                     = 0;
        std::string alive               = handler->GetNuskyString(LANG_ERROR);
        uint32 money                    = 0;
        uint32 xp                       = 0;
        uint32 xptotal                  = 0;
        ...
        handler->PSendSysMessage(LANG_PINFO_PLAYER, target ? "" : handler->GetNuskyString(LANG_OFFLINE), nameLink.c_str(), lowguid);
        ...
        handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetNuskyString(LANG_CHARACTER_GENDER_MALE) : handler->GetNuskyString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(), classStr.c_str());
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        Nusky::RespawnDo u_do;
        Nusky::WorldObjectWorker<Nusky::RespawnDo> worker(player, u_do);

        TypeContainerVisitor<Nusky::WorldObjectWorker<Nusky::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
    ...
    static bool HandleMuteCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string muteReasonStr = handler->GetNuskyString(LANG_NO_REASON);
        if (muteReason != NULL)
        ...
        if (handler->GetSession())
            ...
            muteBy = handler->GetNuskyString(LANG_CONSOLE);
