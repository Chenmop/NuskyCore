...
public:
    ...
    struct npc_lorewalker_cho_pre_norushenAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 p_Diff) override
        {
            ...
            {
                /*
				if (Player* l_Player = me->FindNearestPlayers(30.0f, true))
                {
                    m_Intro = true;
    ...
    struct boss_amagalad_of_corruptionAI : public BossAI
    {
            ...
            if (me->GetHealthPct() <= 50)
            {
                ...
                { 
                    if (me->HealthBelowPctDamaged(m_FrayedPct, p_Damage))
                    {
                        ...
                        if (auto l_Aura = me->GetAura(eSpells::SPELL_FRAYED))
                        {
        ...
        void JustDied(Unit* p_Killer) override
        {
            ...
            if (me->GetMap())
            {
                ...
                {
                    case DifficultyID::DIFFICULTY_10MAN_NORMAL:
                        ...
                        break;
                    case DifficultyID::DIFFICULTY_10MAN_HEROIC:
                        ...
                        break;
                    case DifficultyID::DIFFICULTY_25MAN_NORMAL:
                        ...
                        break;
                    case DifficultyID::DIFFICULTY_25MAN_HEROIC:
...
public:
                    ...
                    if (boss_amagalad_of_corruption::boss_amagalad_of_corruptionAI* l_AlmaglemlinkAI = CAST_AI(boss_amagalad_of_corruption::boss_amagalad_of_corruptionAI, l_Almegalem->GetAI())) // Look Within AI link
                    {
                        if (l_MelinkAI->m_PlayerGUID == 0)
                        {
                            if (p_Creature->IsAIEnabled)
                            {
                                ...
                                /* Need update urgency
								if (p_Player->GetRoleForGroup(p_Player->GetSpecializationId(p_Player->GetActiveSpec())) == ROLE_TANK)
                                {
                                    p_Creature->GetAI()->DoAction(eActions::ACTION_SUMMON_LOOK_WITHIN_TANK);
                                }
                                else if (p_Player->GetRoleForGroup(p_Player->GetSpecializationId(p_Player->GetActiveSpec())) == ROLE_HEALER)
                                {
    ...
    {
        creature_look_within_triggerAI(Creature* p_Creature) : ScriptedAI(p_Creature), summons(me)
        {
            m_Instance = p_Creature->GetInstanceScript();  
        }
        ...
        void Reset() override
        {
            ...
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_DISABLE_MOVE);
        ...
        void DoAction(const int32 p_Action) override
        {
            switch (p_Action)
            {
                case ACTION_COUNT_SERENITY_PHASE_KILLS:
                    ...
                    if (m_SerenityPhaseDeath <= 0)
                    {
                        if (m_PlayerGUID != 0)
                        {
                            ...
                            {
                                l_Player->AddAura(SPELL_PURIFIED, l_Player);
                                l_Player->SetPhaseMask(1, true);
                                l_Player->RemoveAura(SPELL_TEST_OF_SERENITY);
                                l_Player->RemoveAura(SPELL_LOOK_WITHIN_PHASEDPS);
                ...
                case ACTION_LOOK_WITHIN_DEACTIVATE:
                    ...
                    events.ScheduleEvent(EVENT_REACTIVATE_LOOK_WITHIN_TRIGGERS, 100 * IN_MILLISECONDS);
                    break;

                case ACTION_LOOK_WITHIN_ACTIVATE:
                    m_PlayerGUID = 0;
                    m_SerenityPhaseDeath = 0;
                    ...
                    me->AddAura(SPELL_LOOK_WITHIN_VISUAL_BALL, me);            
                    break;
                case ACTION_SUMMON_LOOK_WITHIN_TANK:
                {           
                    if (m_PlayerGUID != 0)
                    {
                        if (Player* l_Player = sObjectAccessor->GetPlayer(*me, m_PlayerGUID))
                        {
                            if (Creature* Titanic = l_Player->SummonCreature(CREATURE_TITANIC_CORRUPTION, g_AmagalamOfCorruptionSpawn, TEMPSUMMON_TIMED_DESPAWN, 60 * IN_MILLISECONDS))
                            {
                                if (Titanic->IsAIEnabled)
                                    Titanic->AI()->SetGUID(me->GetGUID());

                                l_Player->AddAura(SPELL_LOOK_WITHIN_PHASETANK, l_Player);
                                l_Player->AddAura(SPELL_TEST_OF_CONFIDENCE, l_Player);
                                Titanic->SetPhaseMask(l_Player->GetPhaseMask(), true);
                            }
                        }
                    }

                    DoAction(ACTION_LOOK_WITHIN_DEACTIVATE);
                    break;
                }
                case ACTION_SUMMON_LOOK_WITHIN_HEAL:
                {
                    if (m_PlayerGUID != 0)
                    {
                        if (Player* l_Player = sObjectAccessor->GetPlayer(*me, m_PlayerGUID))
                        {
                            ...
                            if (Creature* l_GreaterCorruption = l_Player->SummonCreature(CREATURE_GREATER_CORRUPTION, g_AmagalamOfCorruptionSpawn, TEMPSUMMON_TIMED_DESPAWN, 60 * IN_MILLISECONDS))
                            {
                                ...
                                {
                                    if (Creature * l_Pandaren = l_Player->SummonCreature(l_Entries[l_I], l_Player->GetPositionX(), l_Player->GetPositionY(), l_Player->GetPositionZ(), l_Player->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 60 * IN_MILLISECONDS))
                                    {
                            ...
                            l_Player->AddAura(SPELL_TEST_OF_RELIANCE, l_Player);
                            l_Player->AddAura(SPELL_LOOK_WITHIN_PHASEHEAL, l_Player);
                        }
                    }
                ...
                case ACTION_SUMMON_LOOK_WITHIN_DPS:
                {
                    if (m_PlayerGUID != 0)
                    {
                        if (Player* l_Player = sObjectAccessor->GetPlayer(*me, m_PlayerGUID))
                        {
                            if (l_Player->HasAura(SPELL_CORRUPTION))
                            {
                                /*
                                ...
                                if (auto l_Aura = l_Player->GetAura(SPELL_CORRUPTION))
                                {
                                    int32 bp = l_Aura->GetEffect(0)->GetBaseAmount();
                                    int32 l_ArtificalPower = bp;
                                    ...
                                    for (uint8 i = 0; i <= 3; i++)
                                    {
                                        if (l_ArtificalPower > 40)
                                        {
                                            ...
                                            if (Creature* l_Manifestation = me->SummonCreature(CREATURE_MANIFESTATION_OF_CORRUPTION, g_AmagalamOfCorruptionSpawn, TEMPSUMMON_TIMED_DESPAWN, 58 * IN_MILLISECONDS))
                                            {
                                        ...
                                        else if (l_ArtificalPower > 14)
                                        {
                                            ...
                                            if (Creature* Essence = me->SummonCreature(CREATURE_ESSENCE_OF_CORRUPTION, g_AmagalamOfCorruptionSpawn, TEMPSUMMON_TIMED_DESPAWN, 58 * IN_MILLISECONDS))
                                            {
        ...
        void UpdateAI(const uint32 p_Diff) override
        {
            ...
            if (Creature * l_Almegalem = m_Instance->instance->GetCreature(m_Instance->GetData64(DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
            {
                ...
                switch (events.ExecuteEvent())
                {
                    case EVENT_REACTIVATE_LOOK_WITHIN_TRIGGERS:
                        m_PlayerGUID = 0;
                        m_OriginalPhasemask = 1;
                        m_SerenityPhaseDeath = 0;
...
public:
    ...
    struct creature_look_within_triggerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            m_Succeed = false;
            m_SucceedDiff = 59 * IN_MILLISECONDS;
        }
        ...
        {
            events.ScheduleEvent(eEvents::EVENT_CORRUPTION, 2 * IN_MILLISECONDS);
            events.ScheduleEvent(eEvents::EVENT_TITANIC_SMASH, urand(8 * IN_MILLISECONDS, 12 * IN_MILLISECONDS));
            events.ScheduleEvent(eEvents::EVENT_HURL_CORRUPTION, 20 * IN_MILLISECONDS);
            events.ScheduleEvent(eEvents::EVENT_BURST_OF_CORRUPTION, 30 * IN_MILLISECONDS);
            events.ScheduleEvent(eEvents::EVENT_PIERCING_CORRUPTION, 17 * IN_MILLISECONDS);
        }
                ...
                if (m_Instance != nullptr)
                {
                    if (TempSummon* l_Tempo = me->ToTempSummon())
                    {
                        if (Unit* l_Summoner = l_Tempo->GetSummoner())
                        {
                            ...
                            {
                                if (l_SummonerPlayer->HasAura(SPELL_TITANIC_CORRUPTION))
                                {
                                    if (auto l_Aura = l_SummonerPlayer->GetAura(SPELL_TITANIC_CORRUPTION))
                                    {
                                        l_SummonerPlayer->SetPower(Powers(POWER_ALTERNATE_POWER), 0);
                                    }

                                        l_SummonerPlayer->RemoveAura(SPELL_LOOK_WITHIN_PHASETANK);
                                        l_SummonerPlayer->RemoveAura(SPELL_TEST_OF_CONFIDENCE);
                                        ...
                                        l_SummonerPlayer->AddAura(SPELL_PURIFIED, l_SummonerPlayer);
                                        ...
                                        if (Creature* l_LookWithin = Creature::GetCreature(*me, m_Guid))
                                        {
                                            if (l_LookWithin->IsAIEnabled)
                                                l_LookWithin->GetAI()->DoAction(ACTION_LOOK_WITHIN_ACTIVATE);
                                        }                                  
                                }
                            }
                        }
            ...
            if (Unit* l_Target = me->GetVictim())
            {
                switch (events.ExecuteEvent())
                {
                    case EVENT_TITANIC_SMASH:
                        me->CastSpell(l_Target, SPELL_TITANIC_SMASH);
                        events.ScheduleEvent(EVENT_TITANIC_SMASH, urand(8 * IN_MILLISECONDS, 12 * IN_MILLISECONDS));
                        break;

                    case EVENT_HURL_CORRUPTION:
                        me->CastSpell(l_Target, SPELL_HURL_CORRUPTION);
                        events.ScheduleEvent(EVENT_HURL_CORRUPTION, 45 * IN_MILLISECONDS);
                        break;

                    case EVENT_BURST_OF_CORRUPTION:
                        me->CastSpell(me, SPELL_BURST_OF_CORRUPTION);
                        events.ScheduleEvent(EVENT_BURST_OF_CORRUPTION, 20 * IN_MILLISECONDS);
                        break;

                    case EVENT_PIERCING_CORRUPTION:
                        me->CastSpell(l_Target, SPELL_PIERCING_CORRUPTION);
                        events.ScheduleEvent(EVENT_PIERCING_CORRUPTION, 28 * IN_MILLISECONDS);
                        break;

                    case eEvents::EVENT_CORRUPTION:
                        me->CastSpell(l_Target, SPELL_TITANIC_CORRUPTION);
                        events.ScheduleEvent(EVENT_CORRUPTION, 28 * IN_MILLISECONDS);
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct creature_look_within_triggerAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* p_Attacker)
        {
            events.ScheduleEvent(EVENT_DISHEARTNING_LAUGH, 8 * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_LINGERING_CORRUPTION, 20 * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_BOTTOMLESS_PIT, 30 * IN_MILLISECONDS);
        }
                    ...
                    if (TempSummon* l_Tempo = me->ToTempSummon())
                    {
                        if (Unit* l_Summoner = l_Tempo->GetSummoner())
                        {
                            if (Player* l_SummonerPlayer = sObjectAccessor->GetPlayer(*me, l_Summoner->GetGUID()))
                            {
                                ...
                                l_SummonerPlayer->RemoveAura(SPELL_LOOK_WITHIN_PHASEHEAL);
                                l_SummonerPlayer->RemoveAura(SPELL_TEST_OF_RELIANCE);
                                l_SummonerPlayer->SetPhaseMask(1, true);
                                l_SummonerPlayer->m_Events.KillAllEvents(true); /// stops set original phase from overlapping on other phases
                                l_SummonerPlayer->AddAura(SPELL_PURIFIED, l_SummonerPlayer);
        ...
        void UpdateAI(const uint32 p_Diff)
        {
            ...
            if (Unit* l_Target = me->GetVictim())
            {
                switch (events.ExecuteEvent())
                {
                    case EVENT_DISHEARTNING_LAUGH:
                        DoCast(SPELL_DISHEARTENING_LAUGH);
                        events.ScheduleEvent(EVENT_DISHEARTNING_LAUGH, 20 * IN_MILLISECONDS);
                        break;

                    case EVENT_LINGERING_CORRUPTION:
                        if (Unit* l_RandomTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true))
                            me->CastSpell(l_RandomTarget, SPELL_LINGERING_CORRUPTION);

                        events.ScheduleEvent(EVENT_LINGERING_CORRUPTION, 15 * IN_MILLISECONDS);
                        break;

                    case EVENT_BOTTOMLESS_PIT:
                        events.ScheduleEvent(EVENT_BOTTOMLESS_PIT, 50 * IN_MILLISECONDS);
                        break;
                }
            }
...
public:
    ...
    struct creature_norushen_manifestation_of_corruptionAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* p_Killer)
        {
            if (m_Instance != nullptr)
            {
                if (Creature* l_Manifestation = p_Killer->SummonCreature(CREATURE_UNLEASHED_MANIFESTATION_OF_CORRUPTION, *p_Killer, TEMPSUMMON_MANUAL_DESPAWN))
                ...
                if (Creature* l_LookWithinTrigger = Creature::GetCreature(*me, m_Guid))
                {
                    if (l_LookWithinTrigger->IsAIEnabled)
                        l_LookWithinTrigger->GetAI()->DoAction(ACTION_COUNT_SERENITY_PHASE_KILLS);
                }
                ...
                if (TempSummon* l_Tempo = me->ToTempSummon())
                {
                    if (Unit* l_Summoner = l_Tempo->GetSummoner())
                    {
                        if (Player* l_SummonerPlayer = sObjectAccessor->GetPlayer(*me, l_Summoner->GetGUID()))
                        {
                            if (auto l_Aura = l_SummonerPlayer->GetAura(SPELL_CORRUPTION))
                            {
                                int32 bp = l_Aura->GetEffect(0)->GetBaseAmount();
                                uint32 m_calc = bp - 40;
        ...
        void EnterCombat(Unit* p_Attacker)
        {          
            events.ScheduleEvent(eEvents::EVENT_TEAR_REALITY, urand(8 * IN_MILLISECONDS, 15 * IN_MILLISECONDS));
        }
            ...
            if (Unit* l_Target = me->GetVictim())
            {
                switch (events.ExecuteEvent())
                {
                    case eEvents::EVENT_TEAR_REALITY:
                            me->CastSpell(l_Target, SPELL_TEAR_REALITY);
                            events.ScheduleEvent(EVENT_TEAR_REALITY, urand(8 * IN_MILLISECONDS, 12 * IN_MILLISECONDS));
                            break;
                }
            }
...
public:
    ...
    struct creature_norushen_essnece_of_corruptionAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* p_Attacker) override
        {
            events.ScheduleEvent(eEvents::EVENT_EXPEL_CORRUPTION, urand(6 * IN_MILLISECONDS, 8 * IN_MILLISECONDS));
        }
        ...
        void JustDied(Unit* p_Killer) override
        {
            if (m_Instance != nullptr)
            {
                ...
                if (Creature* l_Essence = p_Killer->SummonCreature(CREATURE_UNLEASHED_ESSENCE_OF_CORRUPTION, *p_Killer, TEMPSUMMON_MANUAL_DESPAWN))
            ...
            if (Unit* l_Target = me->GetVictim())
            {
                switch (events.ExecuteEvent())
                {
                    case EVENT_EXPEL_CORRUPTION:
                        me->CastSpell(l_Target, SPELL_EXPELLED_CORRUPTION);
                        events.ScheduleEvent(EVENT_EXPEL_CORRUPTION, urand(6 * IN_MILLISECONDS, 8 * IN_MILLISECONDS));
                        break;
                }
            }
...
public:
    ...
    struct creature_norushen_manifestation_of_corruptionAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            me->SetReactState(REACT_PASSIVE);        
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE);
            me->CastSpell(me, SPELL_SERENITY_PHASE_MOBS_COSMETIC, true);
            m_VisualHappen = false;
            me->SetDisplayId(11686);
            if (m_Instance != nullptr)
            {
                if (Creature * l_Trigger = m_Instance->instance->GetCreature(m_Instance->GetData64(DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                {
                    l_Trigger->CastSpell(me, SPELL_NORUSHEN_COLOR, true);
                    m_VisualHappen = true;
                    m_DiffVisual = 6 * IN_MILLISECONDS;
                }
            }
            */
        }
        ...
        void EnterCombat(Unit* p_Attacker) override
        {
            events.ScheduleEvent(EVENT_BURST_ANGER, 2 * IN_MILLISECONDS);
        }

        void JustDied(Unit* p_Killer) override
        {
            if (Creature* l_Residual = p_Killer->SummonCreature(CREATURE_RESIDUAL_CORRUPTION, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN))
        ...
        void UpdateAI(const uint32 p_Diff) override
        {
            ...
            */
            ...
            if (Unit* l_Target = me->GetVictim())
            {
                switch (events.ExecuteEvent())
                {
                    case EVENT_BURST_ANGER:
                        me->CastSpell(l_Target, SPELL_BURST_OF_ANGER, false);
                        events.ScheduleEvent(EVENT_BURST_ANGER, 2 * IN_MILLISECONDS);
                        break;
                }
            }
...
public:
    ...
    struct creature_norushen_essnece_of_corruptionAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            me->setFaction(FactionHostile);
            me->SetInCombatWithZone();    
            me->CastSpell(me, SPELL_SERENITY_PHASE_MOBS_COSMETIC);
        }
        ...
        void EnterCombat(Unit* p_Attacker)
        {
            if (m_Instance != nullptr)
            {
                if (Creature * l_Almegalem = m_Instance->instance->GetCreature(m_Instance->GetData64(Data64::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                {
                    me->CastSpell(l_Almegalem, SPELL_FOUL_LINK_AURA);
                }
            }
...
public:
    ...
    struct creature_norushen_levenAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (Creature* l_GreaterCorruption = me->FindNearestCreature(CREATURE_GREATER_CORRUPTION, 100.0f, true))
            {
                if (me->IsAIEnabled)
            ...
            // update with urgency me->RemoveFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_REGENERATE_POWER);
            me->AddAura(SPELL_DAMAGE_DEALER, me);
            me->SetReactState(REACT_AGGRESSIVE);
        ...
        void EnterCombat(Unit* p_Attacker)
        {
            events.ScheduleEvent(EVENT_THREATENING_STRIKES, 2 * IN_MILLISECONDS);
        }

        void MovementInform(uint32 p_Type, uint32 p_Id)
        {
            if (p_Id == MOVEMENT_INFORM_LEAVING_BOTTOMLESS_PIT)
            {
        ...
        void UpdateAI(const uint32 p_Diff)
        {
            ...
            if (!m_Moved)
            {
                if (Creature* l_NearestBottomLessPit = me->FindNearestCreature(CREATURE_BOTTOMLESS_PIT, 1.5f, true))
                {
                    m_Moved = true;
                    ...
                    me->GetMotionMaster()->MovePoint(MOVEMENT_INFORM_LEAVING_BOTTOMLESS_PIT, l_Position);
                }
            }
            ...
            switch (events.ExecuteEvent())
            {
                case eEvents::EVENT_THREATENING_STRIKES:
                {
                    if (Creature* l_GreaterCorruption = me->FindNearestCreature(CREATURE_GREATER_CORRUPTION, 100.0f, true))
                    {
                        ...
                        events.ScheduleEvent(eEvents::EVENT_THREATENING_STRIKES, 2 * IN_MILLISECONDS);
                        break;
                    }
                }
            }
...
public:
    ...
    struct creature_norushen_sunAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* p_Attacker)
        {
            events.ScheduleEvent(EVENT_FIRE_BOLT, 500);
        }

        void MovementInform(uint32 type, uint32 id)
        {
            if (id == MOVEMENT_INFORM_LEAVING_BOTTOMLESS_PIT)
            {
                if (m_BottomLessPitGUID != 0)
                {
                    if (Creature* l_NearestBottomLessPit = Creature::GetCreature(*me, m_BottomLessPitGUID))
        ...
        void UpdateAI(const uint32 p_Diff)
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_FIRE_BOLT:
                {
                    if (Creature* l_GreaterCorruption = me->FindNearestCreature(CREATURE_GREATER_CORRUPTION, 100.0f, true))
                    {
                        me->CastSpell(l_GreaterCorruption, SPELL_FIRE_BOLT);
                        events.ScheduleEvent(EVENT_FIRE_BOLT, 2700);
                        break;
                    }
                }
            }
...
public:
    ...
    struct creature_residual_corruptionAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            me->setFaction(FactionHostile);
            me->SetReactState(REACT_PASSIVE);
            m_IntervalDiff = 2 * IN_MILLISECONDS;      
            me->AddAura(SPELL_RESIDUAL_CORRUPTION_AURA, me); 
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
        }
        ...
        void UpdateAI(const uint32 p_Diff)
        {
            ...
            if (m_Instance != nullptr)
            {
                if (!m_Used)
                {
                    ...
                    {
                        if (auto l_Aura = l_Nearest->GetAura(eSpells::SPELL_CORRUPTION))
                        {
                            if (me->GetMap())
                            {
                                m_Used = true;

                                if (me->GetMap()->GetDifficulty() == DIFFICULTY_10MAN_NORMAL)
                                {
                                    int32 bp = l_Aura->GetEffect(0)->GetBaseAmount();
                                    bp += 40;
...
public:
    ...
    class spell_norushen_icy_fear_AuraScript : public SpellScript
    {
        ...
        void HandleOnHit()
        {
            ...
            if (GetHitUnit()->HasAura(SPELL_CORRUPTION))
            {
                if (auto l_Aura = GetHitUnit()->GetAura(SPELL_CORRUPTION))
                {
                    if (InstanceScript* l_Instance = GetHitUnit()->GetInstanceScript())
                    {
                        if (Creature* l_Amagalem = l_Instance->instance->GetCreature(l_Instance->GetData64(DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION))) ///< Controller, responsible for the boss summoning.
                        {
                            int32 bp = l_Amagalem->GetHealthPct();

                            if (SpellInfo const* l_Info = sSpellMgr->GetSpellInfo(SPELL_ICY_FEAR_DAMAGE))
                            {
                                if (bp <= 0)
                                    return;
...
public:
    ...
    class spell_norushen_unleahsed_anger_SpellScript : public SpellScript
    {
        ...
        void OnHitSetDamage()
        {
            ...
            {
                // Self Doubt
                if (GetHitUnit()->HasAura(SPELL_SELF_DOUBT))
                {
                    auto l_Aura = GetHitUnit()->GetAura(SPELL_SELF_DOUBT);
                    ...
                    GetHitUnit()->AddAura(SPELL_SELF_DOUBT, GetHitUnit());
            }
        }
    ...
    class spell_norushen_blind_hatred_target_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& p_Targets)
        {
            ...
            for (auto itr : l_ListPlayers)
            {
                if (Creature* l_Amagalem = l_Instance->instance->GetCreature(l_Instance->GetData64(DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION))) ///< Controller, responsible for the boss summoning.
                {
                    if (Creature* l_BlindHatredTrigger = l_Amagalem->FindNearestCreature(CREATURE_BLIND_HATRED_TRIGGER, 200.0f, true))
                    {
...
public:
    ...
    class spell_norushen_extract_corruption_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& p_Targets)
        {
            ...
            if (Creature* l_Trigger = l_Instance->instance->GetCreature(l_Instance->GetData64(DATA_NORUSHEN_TRIGGER))) ///< Controller, responsible for the boss summoning.
            {
...
public:
    ...
    class spell_norushen_blind_hatred_SpellScript : public SpellScript
    {
        ...
        void OnAfterCast()
        {
            ...
            {
                if (Creature * l_Almegalem = l_Instance->instance->GetCreature(l_Instance->GetData64(DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                {
                    if (Creature* l_Trigger = l_Instance->instance->GetCreature(l_Instance->GetData64(DATA_NORUSHEN_TRIGGER)))
                    {
                        ...
                        if (Creature* l_BlindHatred = l_Almegalem->SummonCreature(CREATURE_BLIND_HATRED_TRIGGER, l_Almegalem->GetPositionX(), l_Almegalem->GetPositionY(), l_Almegalem->GetPositionZ(), l_Almegalem->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 29 * IN_MILLISECONDS))
                            l_Trigger->CastSpell(l_BlindHatred, SPELL_BLIND_HATRED_FULL, true);
                    }
                }
            }
        }
