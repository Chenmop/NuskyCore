...
public:
    ...
    struct boss_sha_of_prideAI : public BossAI
    {
        ...
        void Reset() override
        {
			...
			// update with urgency me->SetInt32Value(UNIT_FIELD_POWER1, 0);
            me->SetMaxPower(POWER_ENERGY, 100);
            // update with urgency me->SetInt32Value(UNIT_FIELD_MAXPOWER1, 100);
        ...
        void RegeneratePower(Powers power, int32& value)
        {
			...
			// update with urgency me->SetInt32Value(UNIT_FIELD_POWER1, val);
        }
...
public:
    ...
    class spell_power_of_the_titans_spell_Script : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            {
                std::list<Player*> l_ListPlayers;
                Nusky::AnyPlayerInObjectRangeCheck checker(l_NearestPlayer, 10.0f);
                Nusky::PlayerListSearcher<Nusky::AnyPlayerInObjectRangeCheck> searcherBigList(l_NearestPlayer, l_ListPlayers, checker);
...
public:
    ...
    class spell_corrupted_prison_dot_damageAI : public AuraScript {
        ...
        void OnUpdate(AuraEffect const* aurEff)
        {
            ...
            if (InstanceScript* l_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature * l_ShaOfPride = l_Instance->instance->GetCreature(l_Instance->GetData64(DATA_SHA_OF_PRIDE)))
                {
                    ModifyPride(l_ShaOfPride, 5, GetTarget()->ToPlayer()->GetGUID());
                }
            }
